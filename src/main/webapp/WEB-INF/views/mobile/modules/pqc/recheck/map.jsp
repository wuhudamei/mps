...
<div class="g-sign">
	<header>
		<a class="back_btn" href="${ctx }/app/pqc/recheck/recheckList"></a>
		<h2 class="title">复检签到</h2>
	</header><!-- /header -->
	<form  id="signForm" method="post">
		<input id="lat" name="lat" hidden="hidden">
		<input id="lon" name="lon" hidden="hidden">
		<input id="recheckId"  hidden="hidden" name="relatedBusinessId" value="${recheckId}">
		...
		if(isAndroid){
			...
			function locationResult(json){
				...
				if(data.type == "succ"){
					...
					$.post("${ctx}/app/pqc/recheck/pqcsign", $("#signForm").serialize(), function(result){
						if(result == "success"){
							globalUtil.fn.alert("签到成功!",2.0);
							setTimeout('window.location.href="${ctx }/app/pqc/recheck/recheckList"', 2000);
						}else{
...
<body>
	<form action="${ctx }/app/pqc/recheck/pqcsign" method="post"
		...
		<div class="g-sign">
			...
			<input id="locat1" hidden="hidden"> <input id="locat2"
				hidden="hidden"> <input id="inspectId" name="recheckId"
				value="${recheckId}" hidden="hidden"> <input id="distance"
				hidden="hidden" name="signErrorDistance"> <input
			...
			<div class="btns">
				...
				<a class="pos_btn undis" href="javascript:void(0)" onclick="sign()"
					id="qiandao">签到</a>
				<!-- </div> -->
				<!-- <div class="wrong"> -->
		...
		<script type="text/javascript"
			src="${ctxStatic}/mobile/modules/pqc/js/utils/map_init2.js"></script>
		<script type="text/javascript"
...
<script type="text/javascript">
	...
	function ajaxFuc() {
		$.ajax({
			async : false,
			url : "${ctx}/app/pqc/recheck/getAddress?id=${orderId}",
			type : "get",
			dataType : "json",
			success : function(data) {
				//暂时存放经纬度
