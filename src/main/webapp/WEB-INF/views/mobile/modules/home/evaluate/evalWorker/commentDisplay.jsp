...
<body>
	<div class="">
		<header>
			<a class="back_btn" href="${ctx}/app/home/evaluate/evalWorker/list?orderId=${orderId }"></a>
		...
		<form id="jvForm" action="" method="post" enctype="multipart/form-data">
			...
			<section class="pad_top88">
				<p class="font0 locate">
					<span class="font30 col_3 location-text">${evaluateWorker.packageName }</span>
				</p>
				<div class="work-sec font0 bg_f pad_btm24 mar_btm24 border_btm">
					...
					<ul class="pad_top3 pad_left3">
						<c:forEach items="${list }" var="every" varStatus="status">
							<li class="mar_btm46 clearfix a">
								...
								<span class="col_3 font28 flo_left mar_ght52 top-small">${every.indexName }</span>
								<p class="flow star-p">
					...
					<div class="advise-area font0 col_9">
						<textarea class="advise" name="advise" placeholder="请输入您的反馈意见"></textarea>
						<p class="limite font24 col_9"><span class="limite-num">100</span>字</p>
					</div>
					<p class="tips">
						<span class="font24 col_red">提示：</span>
						<span class="font24 col_9">该评价为匿名评分，您的评分直接影响工人评级、结算，我们也会认真考虑您对工人的反馈建议。</span>
					</p>
				</div>
			</section>
		</form>
	...
	<script>
		...
		function evalSubmit(){
			...
			var options ={
					url : "${ctx}/app/home/evaluate/evalWorker/evaluate",
					type: "post",
					success : function(data){
						if(data==0){
							globalUtil.fn.alert("评价成功...",2.0);
							var a ="${ctx}/app/home/evaluate/evalWorker/list";
							setTimeout('window.location.href="'+a+'"', 2000);	
						}
						if(data==1){
							globalUtil.fn.alert("评价失败,已超过评价时间...",2.0);
							var b ="${ctx}/app/home/evaluate/evalWorker/list";
