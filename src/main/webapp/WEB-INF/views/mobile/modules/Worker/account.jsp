...
<body>
	<div class="g-account">
		...
		<section class="account_section">
			...
			<a class="pics shadow" href="${ctx}/app/worker/seePhoto?id=${workTaskPackage.id }&settleStyle=${settleStyle}">查看完工照片</a>
			<h3>工期</h3>
			<ul class="front shadow">
				<div class="normal">
					<li class="clearfix">
						<span>要求工期：</span>
						<p><fmt:formatDate value="${workTaskPackage.planStartdate }" pattern="yyyy-MM-dd" /> 至 <fmt:formatDate value="${workTaskPackage.planEnddate }" pattern="yyyy-MM-dd" /></p>
					</li>
					<li class="clearfix">
						<span>实际工期：</span>
						<p><fmt:formatDate value="${workTaskPackage.actualStartdate }" pattern="yyyy-MM-dd" /> 至 <fmt:formatDate value="${workTaskPackage.actualEnddate }" pattern="yyyy-MM-dd" /></p>
					</li>
				</div>
				<p class="">
			...
			<h3>辅料用量</h3>
			<ul class="front shadow">
				<a class="money" href="${ctx}/app/worker/auxiliaryDetails?id=${workTaskPackage.id }">
					<li class="clearfix">
						<span>辅料金额：</span>
						<p>${bizOrderTaskpackageSettlement.auxiliaryMaterialsAmount}元</p>
					</li>
					<span class="detail">查看详情</span>
				</a>
			</ul>
			<h3>沙子水泥用量</h3>
			<ul class="front shadow">
				<a class="money" href="${ctx}/app/worker/sandDetails?id=${workTaskPackage.id }">
					<li class="clearfix">
						<span>沙子水泥金额：</span>
						<p><c:if test="${bizOrderTaskpackageSettlement.sandCementAmount==null || bizOrderTaskpackageSettlement.sandCementAmount<0}">0.0</c:if><c:if test="${bizOrderTaskpackageSettlement.sandCementAmount>0}">${bizOrderTaskpackageSettlement.sandCementAmount}</c:if>元</p>
					</li>
					<span class="detail">查看详情</span>
				</a>
			</ul>
			...
			<ul class="table shadow">
				...
				<li class="clearfix">
					<span>辅料扣除</span>
					<span>-${bizOrderTaskpackageSettlement.auxiliaryMaterialsAmount}元</span>
				</li>
				<li class="clearfix">
					<span>沙子水泥扣除</span>
					<span>-<c:if test="${bizOrderTaskpackageSettlement.sandCementAmount==null || bizOrderTaskpackageSettlement.sandCementAmount<0}">0.0</c:if><c:if test="${bizOrderTaskpackageSettlement.sandCementAmount>0}">${bizOrderTaskpackageSettlement.sandCementAmount}</c:if>元</span>
				</li>
				<li class="clearfix">
					<span>质保金</span>
				...
				<li class="clearfix">
					<span>工人组结算金额</span>
					<span class="col_red">${bizOrderTaskpackageSettlement.settlementAmount}元</span>
				</li>
			</ul>
		</section>
	</div>
