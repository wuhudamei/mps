...
<body>
	<div class="content_right">
		<header class="header">
			<a class="back_btn" href="${ctx }/app/worker/account?id=${id }&settleStyle=1"></a>
		...
		<div class="accessories">
			<section> 
				<c:forEach items="${procedures }" var = "procedure" varStatus="status">
					<ul>
						...
						<li>
							<span>预算工程量 ：</span>
							<p>${procedure.synthesizePrice }元/${procedure.measurementUnitLabel }*<em>${procedure.budgetNumber }</em>=${procedure.laborAuxiliaryMaterialsBudgetAmount }元</p>
						</li>
						<li>
							<span>结算工程量 ：</span>
							<p>${procedure.synthesizePrice }元/${procedure.measurementUnitLabel }*<em>${procedure.settlementNumber }</em>=${procedure.laborAuxiliaryMaterialsSettleAmount }元</p>
						</li>
					</ul>
				</c:forEach>
			</section>
			<section class="bottom_fixed">
				<p class="total_price lh54">
					工料费预算总金额：<span>${budgetTotalMoney } 元</span>
				</p>
				<p class="total_price lh54">
					工料费结算总金额：<span>${realTotalMoney } 元</span>
				</p>
			</section>
		</div>
