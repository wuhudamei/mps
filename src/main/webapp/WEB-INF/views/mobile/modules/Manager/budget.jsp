        ...
        <section class="pad_top11 section">
            <div class="sec_list">
                ...
                <div class="sec_top">
                    ...
                    <p>
                        <span>任务包状态：</span>
                        <span>${pack.stateName}</span>
                    </p>
                    <p>
                        <span>工料费预算总金额：</span>
                        <span>${pack.laborAuxiliaryMaterialsBudgetAmount}元</span>
                    </p>
                    <p>
                        <span>人工费预算总金额：</span>
                    ...
                    <p>
                        <span>辅料费预算总金额：</span>
                        <span>${pack.auxiliaryMaterialsBudgetAmount}元</span>
                    </p>
                    <p>
                        <span>要求工期：</span>
                        <span><fmt:formatDate value="${pack.startTime}" pattern="yyyy-MM-dd"/> 至 <fmt:formatDate value="${pack.endTime}" pattern="yyyy-MM-dd"/></span>
                    </p>
                    <p>
                 ...
                 <c:forEach items="${procedureList }" var="procedure" varStatus="status">
	                 <c:if test="${procedure.budgetNumber ==0 }"></c:if>
	                 <c:if test="${procedure.budgetNumber !=0 }">
		                 <div class="sec_bot">
		                    <h6>${status.index+1 }、${procedure.procedureName}</h6>
		                    <p>
		                        <span> 工料费预算金额 ：</span><span>${procedure.synthesizePrice}元/${procedure.measurementUnit }*${procedure.budgetNumber}=${procedure.laborAuxiliaryMaterialsBudgetAmount}元</span>
		                    </p>
		                    <p>
		                        <span>辅料费预算金额：</span><span>${procedure.accessoriesPrice}元/${procedure.measurementUnit }*${procedure.budgetNumber}=${procedure.auxiliaryMaterialsBudgetAmount}元</span>
		                    </p>
		                    <p>
		                        <span>人工费预算金额：</span><span>${procedure.laborPrice}元/${procedure.measurementUnit }*${procedure.budgetNumber}=${procedure.laborBudgetAmount}元</span>
		                    </p>
		                    <p class="last_bot">
		                        <span>工序内容：</span>
