...
<script id="contentListTpl" type="text/html">
	<div class="">
		<a class="item zhankai" href="javascript:;">木工工程：石膏板安装</a>
...
<script>
	$(function(){
		var noticeList = [{
  		title: '1.石膏板使用自攻钉固定，间距≦200mm，严禁划破纸面；（拍照）',
  		dw: '户',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：3分/30元', '项目经理（分数/金额）：2分/20元', '施工工人（分数/金额）：3分/30元']
  	},
  	{
  		title: '2.拐角处石膏板采用L或T形套割；（拍照）',
  		dw: '户',
  		zgfs: '违：返工重做并罚款',
  	...
  	{
  		title: '4.石膏板接缝处留5-8mm缝隙，切成V字口；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '5.平整度≤3mm，垂直度≤3mm，方正度≤3mm；',
  		dw: '',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：*', '项目经理（分数/金额）：*', '施工工人（分数/金额）：*']
  	}];
