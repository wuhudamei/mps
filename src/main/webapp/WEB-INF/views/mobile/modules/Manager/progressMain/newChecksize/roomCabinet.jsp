...
<body>
	<div class="g-materi">
		...
		<footer>
			...
			<a href="javascript:void(0)" id="roomCabinet"><span class="btn font28">提交</span></a>
		</footer>
		<!-- 照片弹出层 -->
		<div class="mask mask1 undis">
			<div class="pic_container clearfix" id="foo">
				<%-- <div class="pic_wraper clearfix" id="foo">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			//绑定onclick事件
			$("#roomCabinet").bind('click',roomSubmit);
		});
		...
		$(function(){
			...
			var roomCabinetList = "${roomCabinetList}";
			var sub = roomCabinetList.toString().substr(roomCabinetList.indexOf("[")+1,roomCabinetList.indexOf("]")-1).split(",");
			...
			function add_li(id){
				...
				$("#border_btm"+id).append('<li class="line_hgt clearfix"><span class="font28 col_6 flo_left">复 尺 位置：</span><div class="select_total" style="margin-left: 1.66rem;"><div class="select-area"><div class="select-value"><input hidden="hidden" name="position" class="pos" value="" id="pos'+id+'"/><span class="font24 col_blue mar_left1" id="id'+id+'"></span><i class="select-btn"></i></div><div class="options undis" id="div'+id+'">')
				if(sub.length > 0){
				...
				$(".line_hgt").append('</li>')
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">浴室柜型号：</span><input id="bathroomCabinetModel'+id+'" name="bathroomCabinetModel" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" maxlength="4"><span class="font28 col_6"></span></li>') 
				$(".font0").append('</ul>') 
		...
		function roomSubmit(){
			...
			for(var i=0; i<str.length; i++){
				...
				//alert($("#pos"+i).val());
				//浴室柜型号
				if($("#bathroomCabinetModel"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,浴室柜型号不能为空...",1.0);
					return false;
				}
			}
			
			/**禁用按钮**/
			$("#roomCabinet").css("color","#CCC");
			$("#roomCabinet").unbind("click");
			var options ={
				url : "${ctx}/app/manager/roomSubmitData",
				type: "post",
				success : function(data){
					if(data == 0){
					  ...
					  if(data == 2){
						  globalUtil.fn.alert("提交浴室柜数据失败...",2.0);
					  }
					  if(data == 3){
