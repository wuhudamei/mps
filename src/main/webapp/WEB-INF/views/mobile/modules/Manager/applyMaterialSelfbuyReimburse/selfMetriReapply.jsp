...
<body>
	<div class="">
		...
		<form id="jvForm" action="" method="post" enctype="multipart/form-data">
			...
			<input type="text" hidden="hidden" id="relatedReimburseId" name="relatedReimburseId" value="${relatedReimburseId }">
			<input type="text" hidden="hidden" id="materialId" name="materialId" value="${bizMaterialSelfbuyReimburse.id }">
			<input type="text" hidden="hidden" name="materialSelfbuyId" value="${bizMaterialSelfbuyReimburse.materialSelfbuyId }">
			<section class="pad_top11">
				<ul class="item_lists pl0 bg_f font0">
					...
					<li class="mar_btm42">
						...
						<select class="selectRe font24 col_9 border_d0" disabled="disabled" id="materialSelfbuyId">
							<option value=""></option>
							<option value="${bizMaterialSelfbuyReimburse.materialSelfbuyId }" selected="selected">${bizMaterialSelfbuyReimburse.materialName }</option>
						</select>
					</li>
					<li class="mar_btm42 clearfix">
						<span class="col_6 font30">
							<span class="spanRgt">客户交费金额：</span>
							<input class="inputWrt" placeholder="请输入自采材料施工项，客户交费的金额" type="text" onkeyup="fix_amountthis(this)" onafterpaste="fix_amountthis(this)" onchange="getSettleAmount()" name="customerPayAmount" id="customerPayAmount" value="${bizMaterialSelfbuyReimburse.customerPayAmount }">
					...
					<li class="mar_btm42 clearfix">
						<span class="col_6 font30">
							...
							<input class="inputWrt bg_f2" readonly placeholder="" type="text" id="settleRate"  value="${bizMaterialSelfbuyReimburse.settleRate }">
							<input type="text" hidden="hidden" name="settleRateTwo" id="settleRateTwo"  value="${bizMaterialSelfbuyReimburse.settleRateTwo }">
							<input type="text" hidden="hidden" name="settleStage" id="settleStage"  value="${bizMaterialSelfbuyReimburse.settleStage }">
						</span>
					</li>
					<li class="mar_btm42 clearfix">
						<span class="col_6 font30">
							<span class="spanRgt">实际结算金额：</span>
							<input class="inputWrt bg_f2" readonly placeholder="" type="text" id="settleAmount" name="settleAmount" value="${bizMaterialSelfbuyReimburse.settleAmount }">
							<span class="col_9">元</span>
						</span>
					...
					<li class="clearfix">
						...
						<textarea class="instruc wid420" name="reimburseRemarks" maxlength="30">${bizMaterialSelfbuyReimburse.reimburseRemarks }</textarea>
					</li>
				</ul>
				<div class="bg_f bor_btm">
					...
					<div class="pics font0 clearfix" id="picsSpace">
						...
						<c:forEach items="${picList }" var="p">
							<div class="pic picSpace">
								<img src="${baseUrl}${p.picUrl }" alt="上传图片">
								<a class="del_pic" href="javascript:void(0)"></a>
								<input type="text" hidden="hidden" value="${p.picId }" name="picUrlId">
							</div>
						</c:forEach>
						<%-- <div class="pic">
	...
	<div class="alertMask undis" id="alert3">
		<div class="maskWrapper">
			<p class="col_3 maskTit">提示</p>
			<div class="font28 col_6 maskContent">自采材料报销重新申请成功。</div>
			<div class="maskBtns clearfix twoBtns">
	...
	<script>
		...
		var relatedReimburseId = $("#relatedReimburseId").val();
		...
		function tijiao(){
			...
			var num = $("#picsSpace .picSpace").size()
			if(num==null || num<1){
		...
		function applySubmit(){
			...
			//上报提交表单
			var options ={
					url : "${ctx}/app/manager/applyMaterialSelfbuyReimburse/material_selfbuy_reimburse_submit_reapply",
					type: "post",
					success : function(data){
						$('body').waitMe('hide');
						if(null!=data && data=="0"){
							...
							$("#alertRemarks").text("初次申请的自采材料报销更新失败");
							$("#alert2").show();
						}else if(data=="11"){
		...
		function preview(file) {  
			...
			if (file.files && file.files[0]) {  
				...
				reader.onload = function(evt){
					$('<div class="pic picSpace"><img src="' + evt.target.result + '" onload="compresspic(this,uploadpic)" id="'+shit+'"/><a class="del_pic" href="javascript:void(0)"></a></div>').insertAfter('#camera');
				}    
