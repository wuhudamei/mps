...
<body>
	<div class="wrap">
		...
		<form id="searchForm">
			...
			<input type="text" hidden="hidden" id="pushinstall" name="pushinstall" value="${pushinstall }">
	...
	<script>
		...
		function applySubmit(){
			...
			var options ={
					url: "${ctx}/app/manager/newEnginInstallController/save_push_installation_ajax",
					type: "post",
					success:function(data){
						$('body').waitMe('hide');
						if(null!=data && data=="0"){
							...
							$("#alertRemark").text("您刚刚提交过催促安装，请耐心等待五分钟后再次操作");
		        			$("#alertMask").addClass('_in');
					        $("#alertAlert").addClass('_in');
						}else if(data=="4"){
							$("#alertRemark").text("催促安装内容为空");
		        			$("#alertMask").addClass('_in');
					        $("#alertAlert").addClass('_in');
						}else if(data=="5"){
							$("#alertRemark").text("催促安装保存失败");
		        			$("#alertMask").addClass('_in');
					        $("#alertAlert").addClass('_in');
						}else{
							$("#alertRemark").text("催促安装图片保存失败");
		        			$("#alertMask").addClass('_in');
					        $("#alertAlert").addClass('_in');
						}
					}
