...
<head>
	...
	<link rel="stylesheet" type="text/css" href="${ctxStatic}/mobile/modules/Manager/css/sign/new1/search.css"/>
	<link rel="stylesheet" type="text/css" href="${ctxStatic}/mobile/modules/Manager/css/sign/new1/allBtn.css"/>
...
<body>
	<div class="font0">
		<header>
			...
			<h2 class="title">签到页面</h2>
		</header>
		<section class="pad_top120">
			<c:forEach items="${signList }" var="order">
			
				<div class="shadow mb24">
					<div class="sec bg_f font0 bor_top_e5 bor_dashed clearfix">
						<p class="col_0780ec pt30 pb20">
							<span class="font30 pl40 pr20" style="display:inline-block;word-wrap:break-word;white-space:normal;">
								${order.customerName }-${order.communityName }-${order.buildNumber}-${order.buildUnit}-${order.buildRoom }
							</span>
						</p>
						<ul class="ml52 pr30 bor_top_ddd">
							<c:if test="${order.jzShouldSignDays ne '0'}">
								<li class="rela pb30 clearfix bor_left_d7">
									<i class="signIcon"><!-- <img src="../img/signList.png" alt=""> --></i>
									<p class="pl38 font28 col_3 mb24 pad_rgt3">本月工地基装应签到天数：${order.jzShouldSignDays}天</p>
									<p class="pl38 font28 col_6 mb24 pad_rgt3">应签到次数：${order.jzShouldSignTimes}<c:if test="${order.jzShouldSignTimes ne '没有数据'}">次</c:if></p>
									<p class="pl38 font28 col_6 pad_rgt3">已签到次数：${order.jzAlreadySignTimes}次</p>
								</li>
							</c:if>
							<c:if test="${order.jgShouldSignDays ne '0'}">
								<li class="rela pb30 clearfix bor_left_d7">
									<i class="signIcon"></i>
									<p class="pl38 font28 col_3 mb24 pad_rgt3">本月工地主材应签到天数：${order.jgShouldSignDays}天</p>
									<p class="pl38 font28 col_6 mb24 pad_rgt3">应签到次数：${order.jgShouldSignTimes}<c:if test="${order.jzShouldSignTimes ne '没有数据'}">次</c:if></p>
									<p class="pl38 font28 col_6 pad_rgt3">已签到次数：${order.jgAlreadySignTimes}次</p>
								</li>
							</c:if>
						</ul>
					</div>
					<div class="bg_f">
						<div class="btn_wrapper clearfix">
							...
							<a class="btnBlueTwo" onclick="signDetail('${order.id}')" href="javascript:void(0)">查看详情</a>
						</div>
					</div>
				</div>
			</c:forEach>
	...
	<div class="alertMask undis" id ="alert">
		<div class="maskWrapper">
			...
			<div class="maskBtns clearfix">
				<a class="maskBtn font33 col_f"   href="${ctx}/app/manager/signIndex">确定</a>
			</div>
		</div>
	</div>
	...
	<div class="alertMask undis" id ="alert2">
		<div class="maskWrapper">
			...
			<div class="maskBtns clearfix twoBtns">
				...
				<a class="maskBtn font33 col_0780ec" onclick="closeWin();"  >取消</a>
			</div>
		</div>
	</div>
