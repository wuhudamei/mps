		...
		if(isAndroid){
			...
			function locationResult(json){
				...
				if(data.type == "succ"){
					...
					$.post("${ctx}/app/manager/sign", $("#signForm").serialize(), function(result){
						if(result == "success"){
				...
				if(data.type == "back"){
					window.location.href="${ctx}/app/manager/signIndex";
				}
			}
		}
		if(isiOS){
...
<body>
<form action="${ctx}/app/manager/sign" method="post" id="signForm" enctype="multipart/form-data">
	<div class="g-sign">
		<header>
			<a class="back_btn "  href="${ctx }/app/manager/signIndex"></a>
			<h2 class="title">现场签到</h2>
		...
		<input id="orderId" name="orderId" value="${orderId }">
		<input id="distance" hidden="hidden" name="signDistance">
			<input id="address" hidden="hidden" name="signAddress">
		<div id="map"></div>
		<div class="btns">
	...
	<script type="text/javascript" src="${ctxStatic}/mobile/modules/Manager/js/utils/calcRootFontSize.js"></script>
	<script type="text/javascript" src="${ctxStatic}/mobile/modules/Manager/js/utils/history.js"></script>
	<script type="text/javascript" src="${ctxStatic}/mobile/modules/Manager/js/utils/map_init2.js"></script>
	<script type="text/javascript"
...
$.ajax({
	async: false,
	url : "${ctx}/app/manager/getAddress?id=${orderId}" ,
	type : "get" ,
	dataType: "json",
