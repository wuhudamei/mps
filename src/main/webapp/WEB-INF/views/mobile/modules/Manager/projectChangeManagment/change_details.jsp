...
<body>
	<div class="g-review">
		<header>
			<a class="back_btn"
				href="${ctx }/app/manager/changeManagement/projectChangeList"></a>
			<h2 class="title">施工变更</h2>
		</header>
		<form id="submitFormId">
			<section class="pad_top10 mar_btm50 font0 margin0" id = "myformDiv">
				<div class="mar_left3 mar_rgt3 mar_top1">
					...
					<textarea class="reason_area" name="changeReason" id="changeReason"></textarea>
				</div>
				<!-- 具体项 -->

				<section class="after undis margin0" id="afterSection"></section>
				...
				<div class="g-item">
					...
					<footer>
						<a class="btn_a btn2_1" href="javascript:void(0)"><span
							...
							onclick="hideUndis('addDiv')">取消</span></a>
						<a class="btn_a btn2_2" href="javascript:void(0)">
                            <span class="mask_btn bor_rht" href="javascript:void(0)" onclick="saveItem('addForm')">保存</span>
                        </a>
					</footer>
				</div>
			</div>
	...
	<div class="font0 picWrapper" id="picWrapperDiv">
		<header>
			<a class="back_btn" id="back_btn" href="javascript:;" ></a>
    ...
    <div class="g-mask undis" id = "alertMasksu">
        <div class="alertMask">
            <div class="maskWrapper">
                ...
                <div class="font28 col_6 maskContent align_ctr"><span id = "maskDiv"></span></div>
                <div class="maskBtns clearfix">
                    <a class="maskBtn font33 col_f" href="javascript:;" onclick="hideMyalert()">我知道了</a>
                </div>
            </div>
        </div>
    </div>
	...
	function uploadpic(pic){
		...
		function submit() {
			...
			if (($("#changeReason").val() == "")) {
				...
				$(".after ul").each(
						function() {
							var num = parseFloat($(this).find(
									'input[class*=itemCount]').val());
                            ...
                            if(num == '0'){
                                globalUtil.fn.alert('请输入大于零的变更项数量', 2.0);
                                x++;
							}
				...
				if(count != null && count>=2){
					...
					var options = {
						url : "${ctx}/app/manager/changeManagement/saveChangeForm",
						type : "post",
						success : function(data) {
							if (data == "1") {
								//成功
								globalUtil.fn.alert('提交变更单成功', 2.0);
								window.location.href = "${ctx}/app/manager/changeManagement/projectChangeList";
							} else {
								//失败
                       ...
                       var isMoneyFormatRight = reg.test(num);
                        if(!isMoneyFormatRight){
							...
							if (isNaN(num)) {
								...
								$(this).find('input[class*=totalMoney]').val(
										(num * price).toFixed(2));
							}
			...
			var bindClick = function(liId, selArr, ulName) {
				$(document).on(
						...
						function() {
							...
							var theitem = liId;
							selArr.length = 0;
							$.each($('#' + ulName + ' #' + liId + ' ul label'),
									function(i, v) {
										if ($(v).attr('class') === 'checked') {
											selArr.push($(v).parent().find(
													'input').val());
										}
									});
							console.log(arrObj);
				...
				$(document).on(
						...
						'#' + ulName + ' #' + liId + ' p label',
						function() {
							var b = $(this).parent().parent().find('label')
									.attr('class');
							if (b === 'checked') {
								$(this).parent().parent().find('label').attr(
										'class', '');
								$('#' + ulName + ' #' + liId + ' ul label')
										.attr('class', '');
								selArr.length = 0;
								...
								$(this).parent().parent().find('li').addClass(
										'bg_light');
								$(this).parent().parent().find('label').attr(
										'class', 'checked');
								$('#' + ulName + ' #' + liId + ' ul label')
										.attr('class', 'checked');
								$.each($('#' + ulName + ' #' + liId
										+ ' ul label'), function(i, v) {
									selArr.push($(v).parent().find('input')
											.val());
								});
							}
							console.log(arrObj);
