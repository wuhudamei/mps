...
<script id="contentListTpl" type="text/html">
	<div class="">
		<a class="item zhankai" href="javascript:;">强电工程：线管安装与固定</a>
...
<script>
	$(function(){
		var noticeList = [{
  		title: '1.线管90°转弯为活弯，不得出现死角，线管接头处必须涂刷胶水，且牢固；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '2.线管采用管卡固定，间距≤1000mm/处，牢固，无松动；',
  		dw: '处',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：2分/15元', '项目经理（分数/金额）：2分/10元', '施工工人（分数/金额）：2分/15元']
  	},
  	{
  		title: '3.有防水要求区域（厨房、卫生间、阳台）地面严禁布管布线；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '4.走顶线管必须在水管上方，确保电上水下，严禁水电同槽；',
  		dw: '',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：2分/50元', '项目经理（分数/金额）：2分/30元', '施工工人（分数/金额）：2分/50元']
  	},
  	{
  		title: '5.套线管进底盒内须用锁扣扣紧；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '6.灯头盒需加盖板和波纹软管，使用软管接到灯位时，长度≤1200mm；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	}];
