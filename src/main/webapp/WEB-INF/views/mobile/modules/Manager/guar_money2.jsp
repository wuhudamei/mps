		...
		<section class="font0">
			...
			<ul class="rows">
			     <c:forEach items="${list}" var="money">
					...
					<p class="font28 col_6"><fmt:formatDate value="${money.takeoffDatetime}" pattern="yyyy-MM-dd"/></p> <span
					class="money font48 col_ff9800">+${money.takeoffAmount}</span></li>
			     </c:forEach>
			...
			<ul class="rows bg_f undis">
			    <c:forEach items="${PaidOffineList}" var="money">
			      <a class="row bg_f border_btm pad rgtArrow" href="${ctx}/app/manager/queryManagerPaidMoneyDetail?id=${money.id}">
			...
			<c:forEach items="${usedList}" var="money">
			     <a class="row bg_f border_btm pad rgtArrow" href="${ctx}/app/manager/queryManagerUsedMoneyDetail?id=${money.id}">
