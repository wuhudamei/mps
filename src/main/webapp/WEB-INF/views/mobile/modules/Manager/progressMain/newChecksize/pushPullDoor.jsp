...
<body>
	<div class="g-materi">
		...
		<footer>
			...
			<a href="javascript:void(0)" id="pushDoor"><span class="btn font28">提交</span></a>
		</footer>
		<!-- 照片弹出层 -->
		...
		<div class="mask mask2 undis">
			<div class="big_pic_wraper">
				<img class="big_pic" src="${ctxStatic}/mobile/modules/Manager/images/photo.png" alt="复尺推拉门照片">
			</div>
		</div>
	</div>
	...
	<script>
		$(document).ready(function() {
			//绑定onclick事件
			$("#pushDoor").bind('click',pullDoorSubmit);
		});
		...
		$(function(){
			...
			//获取字典表窗帘
			var pushPullList = "${pushPullList}";
			var sub = pushPullList.toString().substr(pushPullList.indexOf("[")+1,pushPullList.indexOf("]")-1).split(",");
			...
			function add_li(id){
				...
				$(".line_hgt").append('</li>')
				$("#border_btm"+id).append('<li class="line_hgt7 clearfix"><span class="font28 col_6 flo_left">推拉方式：</span><p class="p_right"><span class="radio_span marrgt1"><input type="radio" id="a1' + id +'" name="pushPullStyle'+id+'" value="0"><label class="font26 col_6 mar_rgt10 checked" data-name="meth' + id +'" for="a1' + id +'">套内推拉</label></span><span class="radio_span"><input type="radio" id="b1' + id +'" name="pushPullStyle'+id+'" value="1"><label class="font26 col_6 mar_rgt10" data-name="meth' + id +'" for="b1' + id +'">沿墙推拉</label></span></p></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 clearfix"><span class="font28 col_6 flo_left">包套方式：</span><p class="p_right"><span class="radio_span marrgt1"><input type="radio" id="a2' + id +'" name="packageCover'+id+'" value="0"><label class="font26 col_6 mar_rgt10 checked" data-name="lock' + id +'" for="a2' + id +'">单　　包</label></span><span class="radio_span"><input type="radio" id="b2' + id +'" name="packageCover'+id+'" value="1"><label class="font26 col_6 mar_rgt10" data-name="lock' + id +'" for="b2' + id +'">双　　包</label></span></p></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 clearfix"><span class="font28 col_6 flo_left">开启方向：</span><p class="p_right"><span class="radio_span marrgt1"><input type="radio" id="a3' + id +'" name="openDirection'+id+'" value="0"><label class="font26 col_6 mar_rgt10 checked" data-name="side' + id +'" for="a3' + id +'">右侧移门</label></span><span class="radio_span"><input type="radio" id="b3' + id +'" name="openDirection'+id+'" value="1"><label class="font26 col_6 mar_rgt10" data-name="side' + id +'" for="b3' + id +'">左侧移门</label></span></p></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">门 扇 数 量 ：</span><input id="doorAmount'+id+'" name="doorAmount" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">个</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">新加门垛宽度：</span><input id="newDoorWidth'+id+'" name="newDoorWidth" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">新加门垛高度：</span><input id="newDoorHeight'+id+'" name="newDoorHeight" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">宽　　　　度：</span><input id="width'+id+'" name="width" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">高　　　　度：</span><input id="height'+id+'" name="height" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">厚　　　　度：</span><input id="thickness'+id+'" name="thickness" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" onkeyup="validationData('+id+',this);" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$(".font0").append('</ul>')
		...
		function pullDoorSubmit(){
			...
			for(var i=0; i<str.length; i++){
				...
				//alert($("#pos"+i).val());
				//门扇数量
				if($("#doorAmount"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,门扇数量不能为空...",1.0);
					return false;
				}
				//新加门垛宽度
				if($("#newDoorWidth"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,新加门垛宽度不能为空...",1.0);
					return false;
				}
				//新加门垛高度
				if($("#newDoorHeight"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,新加门垛高度不能为空...",1.0);
					return false;
				}
				//宽度
				if($("#width"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,宽度不能为空...",1.0);
					return false;
				}
				//高度
				if($("#height"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,高度不能为空...",1.0);
					return false;
				}
				//厚度
				if($("#thickness"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,厚度不能为空...",1.0);
					return false;
				}
			}
			/**禁用按钮**/
			$("#pushDoor").css("color","#CCC");
			$("#pushDoor").unbind("click");
			var options ={
				url : "${ctx}/app/manager/pushPullSubmitData",
				type: "post",
				success : function(data){
					...
					if(data == 1){
					  ...
					  if(data == 2){
						  globalUtil.fn.alert("提交推拉门数据失败...",2.0);
					  }
					  if(data == 3){
