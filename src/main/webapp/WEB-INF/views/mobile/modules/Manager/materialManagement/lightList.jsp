...
<body>
	...
	<div class="">
		<header>
			...
			<h2 class="title">申请灯具和五金</h2>
			<a class="tit_rgt" href="javascript:;">申请须知</a>
		</header><!-- /header -->
		<section class="pad_top11">
		<form action="${ctx}/app/manager/applyLightMaterial/lightMaterialList" id="fromSearch">
...
<%-- 					<a id ="send" class="two_btn1" value="${applyFlag}" href="${ctx}/app/manager/applyLightMaterial/applyLightMaterialList/${order.id }">申请筒灯灯带</a> --%>
<%-- 					</c:if> --%>
<%-- 					<c:if test="${ order.isBasicworkCompleted !='0'}"> --%>
<%-- 					<a id ="send" class="one_btn_grey" value="${applyFlag}" href="${ctx}/app/manager/applyLightMaterial/applyLightMaterialList/${order.id }">申请筒灯灯带</a> --%>
<%-- 					</c:if> --%>
					<a class="two_btn2" href="${ctx}/app/manager/applyLightMaterial/record?orderId=${order.id }">申请记录</a>
				</div>
			</div>
				</c:if>
				...
				<div class="btn_wrapper clearfix">
					<c:if test="${order.isBasicworkCompleted =='0'}">
					<a id ="send" class="two_btn1" value="${applyFlag}" href="${ctx}/app/manager/applyLightMaterial/applyLightMaterialList/${order.id }">申请灯具和五金</a>
					</c:if>
					<c:if test="${ order.isBasicworkCompleted !='0'}">
					<a id ="send" class="one_btn_grey" value="${applyFlag}" href="${ctx}/app/manager/applyLightMaterial/applyLightMaterialList/${order.id }">申请灯具和五金</a>
					</c:if>
					<a class="two_btn2" href="${ctx}/app/manager/applyLightMaterial/record?orderId=${order.id }">申请记录</a>
				</div>
			</div>
	...
	<!-- 提示框 -->
				<div style="background:rgba(0,0,0,.4);z-index:9;display:none;" class="g-mask undis" id ="alert" >
					<div id="g-done_ask">
						<p class="first">此订单竣工验收后，
						不允许再次申请</p>
						<p class="second">
							<a href="${ctx}/app/manager/applyLightMaterial/lightMaterialList">确认</a>
						</p>
					</div> 
				</div>
		...
		1.每个订单可以多次申请灯具和五金材料<br/>
		2.单个材料累计申请数量不能超过合同面积计算后的申请上限值<br/>
		3.该订单完成竣工验收后不可再次申请灯具和五金材料<br/>
		以上内容，请在申请前知悉，谢谢！
				</div>
		...
		<div class="alertMask undis" id="applyForRecord">
			<div class="maskWrapper">
				...
				<div class="maskBtns clearfix twoBtns">
					<a class="maskBtnOne font33 col_fdfcfa" href="${ctx}/app/manager/applyLightMaterial/detail?id=${billId}&&orderId=${orderId}">确定</a>
				</div>
			</div>
		</div>
</body>
