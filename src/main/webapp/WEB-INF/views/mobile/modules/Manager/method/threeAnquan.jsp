...
<body>
	<div class="mar_btm300">
		<header>
			<a class="back_btn"  onclick="myhistory.back()" href="${ctx}/app/manager/method/methodSafety"></a>
			<h2 class="title">安全保护</h2>
		</header><!-- /header -->
...
<script id="contentListTpl" type="text/html">
	<div class="">
		<a class="item zhankai" href="javascript:;">强电工程：安全保护</a>
...
<script>
	$(function(){
		var noticeList = [{
  		title: '1.工地临时用电必须使用配电箱，严禁直接连接电动工具；',
  		dw: '',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：8分/100元', '项目经理（分数/金额）：5分/50元', '施工工人（分数/金额）：8分/100元']
  	},
  	{
  		title: '2.每天施工下班，必须关掉电源；',
  		dw: '',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：2分/30元', '项目经理（分数/金额）：1分/20元', '施工工人（分数/金额）：2分/30元']
  	}];
