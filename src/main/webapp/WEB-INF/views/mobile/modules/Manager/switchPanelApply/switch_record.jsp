...
<body>
	<div class="g-budget g-switch_record">
		<header>
			<a class="back_btn" href="${ctx }/app/manager/applySwitchPanel/orderList" ></a>
			<h2 class="title">开关面板申请记录</h2>
				...
				<section class="budget_section">
					...
					<c:forEach items="${purchaseList }" var="purchase" varStatus="status">
						<ul class="shadow">
							...
							<c:if test="${purchase.statusId eq 21}">
								<li class="clearfix">
									<span>废弃原因 ：</span>
									<p>${purchase.statusDescribe }</p>
								</li>
							</c:if>
							<a class="details_btn" href="${ctx }/app/manager/applySwitchPanel/details?id=${purchase.id}&customerInfo=${customerInfo}">查看详情</a>
						</ul>
					</c:forEach>
				</c:if>
			</section>
