...
<script id="contentListTpl" type="text/html">
	<div class="">
		<a class="item zhankai" href="javascript:;">给水工程：卫浴龙头安装</a>
...
<script>
	$(function(){
		var noticeList = [{
  		title: '1.所有出水口有软管连接位置，必须安装角阀，才能安装软管；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '2.安装前必须做水流畅通试验，至少5分钟，冲走管内灰尘，避免堵塞淋浴龙头；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '2.龙头安装必须牢固，无松动。安装工具必须先用布料包扎，避免刮花龙头。安装完后必须做畅通试验；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	}];
