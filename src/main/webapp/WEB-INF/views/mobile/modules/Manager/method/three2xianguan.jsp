...
<body>
	<div class="mar_btm300">
		<header>
			<a class="back_btn"  onclick="myhistory.back()" href="${ctx}/app/manager/method/methodBuildTwo"></a>
...
<script id="contentListTpl" type="text/html">
	<div class="">
		<a class="item zhankai" href="javascript:;">弱电工程：线管安装与固定</a>
...
<script>
	$(function(){
		var noticeList = [{
  		title: '1.强弱电布管布线必须分开，间距≥300mm；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	},
  	{
  		title: '2.弱电线路必须从弱电箱到最终端，中途不得剪断；',
  		dw: '',
  		zgfs: '违：返工重做并罚款',
  		bcfr: ['质检员（分数/金额）：5元', '项目经理（分数/金额）：10元', '施工工人（分数/金额）：20元']
  	},
  	{
  		title: '3.弱电总箱必须预留强电电源；',
  		dw: '',
  		zgfs: '违：返工重做',
  		bcfr: ['质检员（分数/金额）：0', '项目经理（分数/金额）：0', '施工工人（分数/金额）：0']
  	}];
