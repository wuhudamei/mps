...
<body>
	<div class="g-materi">
		...
		<footer>
			...
			<a href="javascript:void(0)" id="curtain"><span class="btn font28">提交</span></a>
		</footer>
		<!-- 照片弹出层 -->
		...
		<div class="mask mask2 undis">
			<div class="big_pic_wraper">
				<img class="big_pic" src="${ctxStatic}/mobile/modules/Manager/images/photo.png" alt="复尺窗帘照片">
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			//绑定onclick事件
			$("#curtain").bind('click',curtainSubmit);
		});	
		...
		$(function(){
			...
			//获取字典表窗帘
			var curtainList = "${curtainList}";
			var sub = curtainList.toString().substr(curtainList.indexOf("[")+1,curtainList.indexOf("]")-1).split(",");
			...
			function add_li(id){
				...
				$(".line_hgt").append('</li>')
				$("#border_btm"+id).append('<li class="line_hgt7 clearfix"><span class="font28 col_6 flo_left">窗帘杆类型：</span><p class="p_right"><span class="radio_span marrgt1"><input type="radio" id="a1' + id +'" name="poleType'+id+'" value="0"><label class="font26 col_6 mar_rgt10 checked" data-name="meth' + id +'" for="a1' + id +'">罗马杆</label></span><span class="radio_span"><input type="radio" id="b1' + id +'" name="poleType'+id+'" value="1"><label class="font26 col_6 mar_rgt10" data-name="meth' + id +'" for="b1' + id +'">滑道</label></span></p></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">窗帘杆长：</span><input id="poleLength'+id+'" name="poleLength" onkeyup="validationData('+id+',this);" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$("#border_btm"+id).append('<li class="line_hgt7 font0 clearfix"><span class="font28 col_6">帘布高度：</span><input id="clothHeight'+id+'" name="clothHeight" onkeyup="validationData('+id+',this);" class="font28 col_6 wid_314 blue_border pad_top1 pad_btm1 mar_rgt2" type="text" maxlength="4"><span class="font28 col_6">mm</span></li>') 
				$(".font0").append('</ul>')
			...
			$(document).on('click','.move_btn',function(){//删除按钮
				$(this).parent().remove();
				var id = $(this).attr("id");
		...
		function curtainSubmit(){
			...
			for(var i=0; i<str.length; i++){
				...
				//alert($("#pos"+i).val());
				//窗帘杆长
				if($("#poleLength"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,窗帘杆长不能为空...",1.0);
					return false;
				}
				//帘布高度
				if($("#clothHeight"+str[i]).val() == ""){
					globalUtil.fn.alert("复尺位置：【"+$("#id"+str[i]).text()+"】,帘布高度不能为空...",1.0);
					return false;
				}
			}
			
			/**禁用按钮**/
			$("#curtain").css("color","#CCC");
			$("#curtain").unbind("click");
			var options ={
				url : "${ctx}/app/manager/curainSubmitData",
				type: "post",
				success : function(data){
					...
					if(data == 1){
					  ...
					  if(data == 2){
						  globalUtil.fn.alert("提交窗帘数据失败...",2.0);
					  }
					  if(data == 3){
