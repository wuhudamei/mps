...
<body>
	<div class="">
		<header>
			<a class="back_btn" href="${ctx}/app/manager/problem/siteDesign/menu"></a>
			<h2 class="title">设计问题上报</h2>
		</header><!-- /header -->
	...
	<div style="background:rgba(0,0,0,.4);z-index:9;" class="g-mask undis" id ="subReport">
		<div class="alertMask">
			<div class="maskWrapper">
				...
				<div class="font28 col_6 maskContent" id="messageContext">您刚刚提交过工地设计问题上报，请耐心等待五分钟后再次操作</div>
...
<script type="text/javascript">
		...
		function searchPurchaseList(){
	        ...
	        $.ajax({
	            url: "${ctx}/app/manager/problem/siteDesign/problem_sign_design_ajax_list",
	            type: "post",
	            data:param,
		            	...
		            	for(var v=0;v<data.length;v++){
							...
							html +=	'<li class="mar_btm24 clearfix">'+
									...
									'<div class="btn_wrapper clearfix">'+
										...
										'<a class="two_btn2" href="${ctx}/app/manager/problem/siteDesign/signDesignproblemRecord?orderId='+data[v].orderId+'">上报记录</a>'+
									'</div>'+
								'</div>';
		            	}
	            	}
			 ...
			 $.ajax({
		            ...
		            success: function(data){
		            	if(null!=data && data == "0"){
		            		...
		            		window.location.href = "${ctx}/app/manager/problem/siteDesign/problem_sign_design?orderId="+orderId;
		            	
		            	}else if(data == "1"){
		            		$("#messageContext").text("订单id传送失败");
		            		$("#subReport").show();
		            	}else{
		            		$("#messageContext").text("您刚刚提交过工地设计问题上报，请耐心等待五分钟后再次操作");
		            		$("#subReport").show();
		            	}
		            }
			 })
