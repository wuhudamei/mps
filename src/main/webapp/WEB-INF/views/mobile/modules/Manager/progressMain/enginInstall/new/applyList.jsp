...
<body>
				...
				<div class="sec font0 bor_top_e5 bg_f clearfix">
					<ul class="pad_top3">
						<li class="mb30 rela pl30 pr30 clearfix">
							...
							<a class="font28 col_378afa noReason spread" >驳回原因</a>
						</li>
						<div class="reasonDetail undis">
							<div class="pb20">
								<span class="col_a7a6a6 font30 fl">驳回原因：</span>
								<p class="font30 col_a7a6a6 flow">${orderInstallPlan.rejectRemarksName }</p>
							</div>
							<div>
								<span class="col_a7a6a6 font30 fl pl2em">备注：</span>
								<p class="font30 col_a7a6a6 flow">${orderInstallPlan.rejectBusinessRemarks }</p>
							</div>
						</div>
						...
						<c:if test="${orderInstallPlan.isShowInstallDescription eq 1 }">
							<li class="mb30 pl30 pr30 rela clearfix">
								...
								<textarea class="instruc limitText" placeholder="请您按照下方提示输入数据！" maxlength="200" id="installRemarks" name="installRemarks">${orderInstallPlan.installRequire}</textarea>
	...
	<script>
        ...
        $(document).on('click', '.noReason', function(){
        	if($(this).hasClass('spread')){
        		$(this).removeClass('spread').addClass('fold');
				$('.reasonDetail').removeClass('undis');
			}else{
        		$(this).removeClass('fold').addClass('spread');
				$('.reasonDetail').addClass('undis');
			}
		});
		...
		function backlast(){
			window.location.href = "${ctx }/app/manager/newEnginInstallController/installApplication?id="+${order.id};
		}
