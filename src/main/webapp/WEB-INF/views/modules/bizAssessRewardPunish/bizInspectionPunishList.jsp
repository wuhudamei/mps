...
<body>
	<ul class="nav nav-tabs">
		<li class="active">
			<ul class="nav nav-tabs">
				<li class="active"><a href="#">项目经理巡检罚款列表</a></li>
			</ul>
		<li><a
			href="${ctx}/bizAssessRewardPunish/bizAssessRewardPunish/openBizAssessPunishForm?isMonthInspection=1">项目经理巡检罚款添加</a>
		</li>
		</li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizAssessRewardPunish"
		action="${ctx}/bizAssessRewardPunish/bizAssessRewardPunish/queryBizAssessPunish"
		method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden"
			value="${page.pageSize}" />
		<input type="hidden" name="isRewardOrPunish" value="2"/>
		...
		<ul class="ul-form">
			...
			<li><label>罚款日期：</label> <input name="startDate" id="startDate"
		...
		<tbody>
			<c:forEach items="${page.list}" var="bizAssessRewardPunish">
				<tr>
					...
					<td><a href="${ctx}/bizAssessRewardPunish/bizAssessRewardPunish/detail?ids=${bizAssessRewardPunish.ids}">详情</a>
					    <c:if test="${bizAssessRewardPunish.rewardPunishStatus == 1}">
							<a href="${ctx}/bizAssessRewardPunish/bizAssessRewardPunish/openBizAssessPunishForm?ids=${bizAssessRewardPunish.ids}">修改</a>
							<a href="#" onclick="del('${bizAssessRewardPunish.ids}')">删除</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
