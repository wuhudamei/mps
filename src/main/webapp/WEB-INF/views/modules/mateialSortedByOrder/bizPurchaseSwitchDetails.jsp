		...
		<div>
			...
			<div class="row clearfix">
				...
				<table style="width:100%" align="center" valign="center" class="table table-striped table-bordered table-condensed">
					<tr>
						<td style="width:50%"><label>订单编号：</label>${bizMaterialsSwitchByOrder.orderNumber}</td>
						<td style="width:50%"><label>工程模式：</label>${fns:getDictLabel(bizMaterialsSwitchByOrder.projectMode,'project_mode', '')}</td>
					</tr>
					<tr>
						<td style="width:50%"><label>区域：</label>${bizMaterialsSwitchByOrder.enginDepartName}</td>
						<td style="width:50%"><label>项目经理：</label>${bizMaterialsSwitchByOrder.itemManager}</td>
					</tr>
					<tr>
						<td style="width:50%"><label>申请开关面板次数：</label>${bizMaterialsSwitchByOrder.applyTimes}</td>
						<td style="width:50%"><label>订单状态：</label>${bizMaterialsSwitchByOrder.orderStatus}</td>
					</tr>
					<tr>
						<td style="width:50%"><label>客户姓名：</label>${bizMaterialsSwitchByOrder.customerName}</td>
						<td style="width:50%"><label>客户地址：</label>${bizMaterialsSwitchByOrder.customerAddress}</td>
					</tr>
				</table>
			</div>
			<div class="row clearfix">
                ...
                <div>
					...
					<c:forEach items="${findMingxiByOrderId}" var="findMingxiByOrderId" varStatus="status">
						<tr>
							...
							<td>
								${findMingxiByOrderId.auxiMateCode}
							</td>
							<td>
