                 ...
                 success:function(data){
                     if(data == null){
                         ...
                         for(var i=0;i<data.length;i++){
                             var sec = "";
                             if('${bizMaterialsWallAndFloorByOrder.enginDepartId}' == data[i].value){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bizMaterialSortedByOrder/materialWallAndFloorByOrder/WallAndFloorList">订单墙地砖统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizMaterialsWallAndFloorByOrder" action="${ctx}/bizMaterialSortedByOrder/materialWallAndFloorByOrder/WallAndFloorList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li ><label>采购单状态：</label>
				...
				<c:forEach items="${listStatus}" var="dict">
					<input type="checkbox" name="status" id="status" value="${dict.value}"  onclick="ChkSonClick('status','chkAll')"  <c:if test="${fns:checkIsExits(bizMaterialsWallAndFloorByOrder.status,dict.value)}"> checked="checked"</c:if>/>${dict.label}&nbsp;
				</c:forEach>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizPurchaseVo">
			<tr>
				...
				<td>
					<a href="${ctx}/bizMaterialSortedByOrder/materialWallAndFloorByOrder/detail?orderId=${bizPurchaseVo.orderId}">
							详情
					</a>
				</td>
			</tr>
