...
<head>
	...
	<script type="text/javascript">
		...
		function exportExcel(){
	        $("#searchForm").attr("action", "${ctx}/bizMaterialSortedByOrder/materialSortedByOrder/export");
	        $("#searchForm").submit();
	        $("#searchForm").attr("action", "${ctx}/bizMaterialSortedByOrder/materialSortedByOrder/AccessoriesList");
		}
         //加载区域信息
                 ...
                 success:function(data){
                     if(data == null){
                         ...
                         for(var i=0;i<data.length;i++){
                             var sec = "";
                             if('${bizMaterialsSortedByOrder.enginDepartId}' == data[i].value){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bizMaterialSortedByOrder/materialSortedByOrder/AccessoriesList">订单辅料统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizMaterialsSortedByOrder" action="${ctx}/bizMaterialSortedByOrder/materialSortedByOrder/AccessoriesList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>区域：</label>
				<form:select path="enginDepartId" class="input-medium needClear">
					...
					<form:option value="${bizMaterialsSortedByOrder.enginDepartId }" label="${bizMaterialsSortedByOrder.enginDepartName }" />
				</form:select>
			</li>
			<li><label>订单编号：</label>
			...
			<li><label style="width: 150px;">项目经理收货日期 ：</label>
				<input name="finalReceiveTimeBegin" type="text" id="finalReceiveTimeBegin" readonly="readonly" maxlength="20" class="input-medium Wdate needClear"
					value="<fmt:formatDate value="${bizMaterialsSortedByOrder.finalReceiveTimeBegin}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				...
				<input name="finalReceiveTimeEnd" type="text" id="finalReceiveTimeEnd" readonly="readonly" maxlength="20" class="input-medium Wdate needClear"
					value="<fmt:formatDate value="${bizMaterialsSortedByOrder.finalReceiveTimeEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"
            ...
            <li><label style="width: 150px;">项目经理申请日期 ：</label>
                <input name="applyTimeBegin" type="text" id="applyTimeBegin" readonly="readonly" maxlength="20" class="input-medium Wdate needClear"
                       value="<fmt:formatDate value="${bizMaterialsSortedByOrder.applyTimeBegin}" pattern="yyyy-MM-dd HH:mm:ss"/>"
                ...
                <input name="applyTimeEnd" type="text" id="applyTimeEnd" readonly="readonly" maxlength="20" class="input-medium Wdate needClear"
                       value="<fmt:formatDate value="${bizMaterialsSortedByOrder.applyTimeEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"
            ...
            <li ><label>采购单状态：</label>
                ...
                <c:forEach items="${listStatus}" var="dict">
                    <input type="checkbox" name="status" id="status" value="${dict.value}"  onclick="ChkSonClick('status','chkAll')"  <c:if test="${fns:checkIsExits(bizMaterialsSortedByOrder.status,dict.value)}"> checked="checked"</c:if>/>${dict.label}&nbsp;
                </c:forEach>
            </li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizPurchaseVo">
			<tr>
				...
				<td>
					<a href="${ctx}/bizMaterialSortedByOrder/materialSortedByOrder/detail?orderId=${bizPurchaseVo.orderId}">
							详情
					</a>
				</td>
			</tr>
