...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/modules/orderInstallPlanAdjustment/web/orderInstallPlanAdjustment/preInstallPlanQueryInstallList">主材可申请安装日期查询</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="orderInstallPlanAdjustment" action="${ctx}/modules/orderInstallPlanAdjustment/web/orderInstallPlanAdjustment/installPlanQueryInstallList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="orderInstallPlanAdjustment">
			<tr>
				...
				<td>
					<fmt:formatDate type="date" value="${orderInstallPlanAdjustment.planIntoDate }"/>
				</td>
	...
	<script type="text/javascript">
		...
		function dealAlertSure(){
			...
			if(newPlanApplyDate == ""){
				$("#informBoxRemark").text("请输入可申请安装日期");
        		$("#informBox").show();
        		return;
			}
			$.ajax({
				url: "${ctx}/modules/orderInstallPlanAdjustment/web/orderInstallPlanAdjustment/save_order_install_plan_install_date_apply",
	            type: "post",
	            data:{
	            		id:idGlobal,
	            ...
	            success: function(data) {
	            	...
	            	if(null!=data && data=="0"){
	            		$("#informBoxRemarkTwo").text("可申请安装日期修改成功");
	            		$("#informBoxTwo").show();
	            	}else if(data=="2"){
	            		$("#informBoxRemark").text("您刚刚该安装项计划的可申请安装日期，请耐心等待五分钟后再次操作");
	            		$("#informBox").show();
	            	}else if(data=="3"){
	            		...
	            		$("#informBoxRemark").text("请修改可申请安装日期");
	            		$("#informBox").show();
	            	}else{
	            		$("#informBoxRemark").text("可申请安装日期修改失败");
	            		$("#informBox").show();
	            	}
	            }
	    	})
