...
<head>
	...
	<script type="text/javascript">
		...
		function exportExcel(){
			...
			// 判断区域
			if('${bizTraditionOrder.engineDepartId}' != $("#engineDepartId").val()){
				alertx("请先查询后，再导出");
				return;
			}
			//判断片区
			if('${bizTraditionOrder.area}' != $("#area").val()){
				alertx("请先查询后，再导出");
				return false;
			}
			//实际开工日期  开始
...
<body>
	...
	<form:form id="searchForm" modelAttribute="bizTraditionOrder"  method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>片区：</label>
				<form:input path="area" htmlEscape="false" maxlength="11" class="input-medium"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th colspan="2" style="text-align: center">设计师</th>
		...
		<c:forEach items="${page.list}" var="nodePlan">
			<tr>
				...
				<td>${nodePlan.designerName}</td>
				<td>${nodePlan.designerPhone}</td>
				<td>${nodePlan.inspector}</td>
				<td>${nodePlan.inspectorPhone}</td>
