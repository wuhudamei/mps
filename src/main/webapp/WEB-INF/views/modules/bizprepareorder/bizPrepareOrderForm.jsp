...
<script type="text/javascript">
	$(document).ready(function() {
		...
		//$("#s2id_designerName .select2-chosen").html($("#empName").val());
		
		/* $.ajax({
			url : "${ctx}/bizprepareorder/bizPrepareOrder/checkForm?empName="+""+"&empPhone="
					 +"",
			type : "get",
			success : function(data) {
				...
				for(var i=0;i<data.length;i++){
					if(data[i].realname==empName){
						$("#s2id_designerName .select2-chosen").html(data[i].realname);
						option+="<option value='"+data[i].phone+"' selected='selected' >"+data[i].realname+"</option>"
					}else{
						option+="<option value='"+data[i].phone+"' >"+data[i].realname+"</option>"
					}
				}
				$("#designerName").html(option);
			}
		}) */
		...
		var getOrderDatetime = $("#getOrderDatetime").val();
		if (getOrderDatetime == null || "" == getOrderDatetime) {
			//默认接单时间为当前系统时间
			var now = jQuery.now();
			$("#getOrderDatetime").val(format(now,'yyyy-MM-dd'))	
		...
		$("#inputForm").validate({
			submitHandler : function(form) {
				...
				//设计师校验
				var empName = $("#designerName").val();
				var empPhone = $("#designerPhone").val();
				
				var flag = false;
				$.ajax({
					url : "${ctx}/bizprepareorder/bizPrepareOrder/checkForm?empName="+empName+"&empPhone="
							 +empPhone,
					type : "get",
					success : function(data) {
						for(var i=0;i<data.length;i++){
							if(data[i].realname==empName&&data[i].phone==empPhone){
								flag = true;						
								break;
							}
						}
						if(flag){
							//审计员校验
							var auditorEmployeeId = $("#auditorEmployeeId").val();
							var auditorName = $("#auditorName").val();
							var auditorPhone = $("#auditorPhone").val();
							var auditorNameStandard = $("#auditorName"+auditorEmployeeId).val();
							var auditorPhoneStandard = $("#auditorPhone"+auditorEmployeeId).val()
							if(auditorName == auditorNameStandard && auditorPhone==auditorPhoneStandard){
								loading('正在提交，请稍等...');
								$("#btnSubmit").attr("disabled",true);
								form.submit();
							}else{
								alertx('审计员姓名或电话与系统不匹配，请重新选择审计员!');
							}
						}else{
							alertx('设计师姓名或电话与系统不匹配，请重新选择设计师!');
						}
					}
				})
			},
	...
	function changeCheck(obj){
		$("#designerName").val($("#s2id_empNameSel .select2-chosen").text());
		$("#designerPhone").val($(obj).val());
		$("#designerPhone")[0].defaultValue=$(obj).val()
	}
	//审计员
	function changeAuditor(){
		var auditorEmployeeId = $("#auditorEmployeeId").val();
		$("#auditorName").val($("#auditorName"+auditorEmployeeId).val());
		$("#auditorPhone").val($("#auditorPhone"+auditorEmployeeId).val());
	}
	...
	*/
	...
	function dateAutoFill(){
		...
		if(startTime!=""&&startTime!=undefined&&startTime!=NaN&&endTime!=undefined&&endTime!=NaN&&endTime!=""){
			$("#contractDays").val(getDays(startTime,endTime));
	...
	function checkPhone() {
		var phone = $("#phone").val();
		if(phone.isNumber()&& phone.length==11){
		}else{
	...
	function formSubmit() {
		...
		$("select[name='province'] option:selected").val(province);
		var city = $("select[name='city'] option:selected").text();
		$("select[name='city'] option:selected").val(city);
		var county = $("select[name='county'] option:selected").text();
		$("select[name='county'] option:selected").val(county);
	...
	function findInstallItem(){
		...
		var projectModeValue = $(":radio[name='projectMode'][checked='checked']").val();
				...
				success : function(data) {
					if (null!= data && data.length > 0) {
						...
						for (var v = 0; v < data.length; v++) {
							if('${bizPrepareOrder.enginDepartId}' == data[v].engineDepartId){
		...
		var html2 = '<option value="">---请选择地区---</option>';
		$.ajax({
			url : "${ctx}/order/order/findacceptAreaBystoreIdAndProjectMode?storeId="
					+ storeId + "&projectModeValue=" + projectModeValue,
			type : "get",
			success : function(data) {
				if (data.length > 0) {
					for (var v = 0; v < data.length; v++) {
						...
						if('${bizPrepareOrder.bizOrderAcceptArea}' == data[v].acceptAreaId){
							$("#s2id_acceptAreaId .select2-chosen").html(data[v].acceptArea);
							html2 = html2 + "<option value='"+data[v].acceptAreaId+"' selected='selected'>"+data[v].acceptArea+"</option>";
						}else{
							html2 = html2 + "<option value='"+data[v].acceptAreaId+"'>"+data[v].acceptArea+"</option>";
						}
						
						
						/* html2 +='<option value="'+data[v].acceptAreaId+'">'+data[v].acceptArea+'</option>'  */
					}
					$("#bizOrderAcceptArea").html(html2);
				} else {
					$("#bizOrderAcceptArea").html(html2);
				}
			}
     ...
     * @constructor
     */
	 function  FloorAutoWrite() {
		 	
	     var roomVal = $("#buildRoom").val();
	     var  floorVal="";
		 if(roomVal.length>1){
	         for(var v =roomVal.length-1;v>-1;v--){
				 if(roomVal[v]==0){
		             for(var w=0;w<v;w++){
		                floorVal=floorVal+roomVal[w];
					 }
					 if(!isNaN(floorVal)){
			             $("#floor").val(floorVal);
						 break;
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/bizprepareorder/bizPrepareOrder/form?id=${bizPrepareOrder.id}">预备订单修改</a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="bizPrepareOrder" action="${ctx}/bizprepareorder/bizPrepareOrder/save" method="post" class="form-horizontal">
		<form:hidden path="id" />
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="storeId" class="input-medium required" onchange="findInstallItem()">
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:radiobuttons path="projectMode" items="${fns:getDictList('project_mode')}" itemLabel="label"
								itemValue="value" htmlEscape="false" class=" required" onchange="findInstallItem()" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input readOnly="true"  path="orderNumber" htmlEscape="false" maxlength="100"
								class="input-xlarge required" id="orderNumber" />
							<span class="help-inline"><font color="red">*</font></span> 
						</div>
					</div>
				</td>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<select name="enginDepartId" style="width: 170px" class = "required" id="enginDepartId">
								<c:if test="${bizPrepareOrder.enginDepartId!=null }">
									<option value="${bizPrepareOrder.enginDepartId }" selected="selected">${bizPrepareOrder.enginDepartName }</option>
								</c:if>
								<c:if test="${bizPrepareOrder.enginDepartId==null }">
							...
							<%-- <form:select path="enginDepartId" class="input-medium ">
								<form:option value="" label="" />
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input  readOnly="true"  path="customerName" htmlEscape="false" maxlength="16" class="input-medium required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input  readOnly="true"  path="customerPhone" htmlEscape="false"
								maxlength="11" class="input-medium required" id="phone"
								onblur="checkPhone()" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
			...
			<tr>
				<td style="width: 260px;">
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="saleType" htmlEscape="false" maxlength="32" class="input-xlarge required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="longwayAmount" htmlEscape="false" maxlength="100" class="input-medium number" onkeyup="fix_amountthis(this)" onafterpaste="fix_amountthis(this)"/>
						</div>
					</div>
				</td> 
			</tr>
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="communityName" htmlEscape="false" maxlength="32" class="input-medium required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="buildNumber" htmlEscape="false" maxlength="4" class="input-mini required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="buildUnit" htmlEscape="false" maxlength="4" class="input-mini required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="buildRoom" htmlEscape="false" maxlength="4" class="input-mini required" onchange="FloorAutoWrite()" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="floor" htmlEscape="false" maxlength="4" class="input-mini required" 
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="contractAmount" htmlEscape="false" maxlength="11"
								class="input-medium number required" onkeyup="fix_amountthis_two(this)" onafterpaste="fix_amountthis_two(this)" />
						<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="detailAddress" htmlEscape="false" maxlength="32" class="input-xlarge required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<select id="province" onchange="setCity(this.value);getArea();"
								name="province" style="width: 170px" runat="server" class="required">
								<c:if test="${bizPrepareOrder.province!=null }">
									<option value="${bizPrepareOrder.province }">${bizPrepareOrder.province }</option>
								</c:if>
								<c:if test="${bizPrepareOrder.province ==null}">
									<option value=""></option>
								</c:if>
					...
					<div class="control-group">
						...
						<div class="controls">
							<select id="city" onchange="setCounty(this.value);getArea();"
								name="city" style="width: 170px" runat="server" class="required">
								<c:if test="${bizPrepareOrder.city!=null }">
									<option value="${bizPrepareOrder.city }" selected="selected">${bizPrepareOrder.city }</option>
								</c:if>
								<c:if test="${bizPrepareOrder.city==null }">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select id="county" name="county" style="width: 170px"
								runat="server" onchange="getArea();" class="required">
								<c:if test="${bizPrepareOrder.county!=null }">
									<option value="${bizPrepareOrder.county }" selected="selected">${bizPrepareOrder.county }</option>
								</c:if>
								<c:if test="${bizPrepareOrder.county==null }">
									<option value="" selected="selected"></option>
								</c:if>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:radiobuttons path="houseIsNew" items="${fns:getDictList('biz_is_new_house')}" itemLabel="label" itemValue="value" htmlEscape="false" class=" required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:radiobuttons path="isElevator" items="${fns:getDictList('is_elevator')}" itemLabel="label" itemValue="value" htmlEscape="false" class=" required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:select id="empNameSel" path="" onchange="changeCheck(this)"  style="width: 170px" class="input-xlarge required" >
								<form:option value="" ></form:option>
								<c:forEach items="${empList}" var = "employee">
									<c:if test="${bizPrepareOrder.designerName eq employee.realname }">
										<form:option value="${employee.phone }" selected="selected">${employee.realname }</form:option>
									</c:if>
									<c:if test="${bizPrepareOrder.designerName ne employee.realname }">
										<form:option value="${employee.phone }" >${employee.realname }</form:option>
									</c:if>
								</c:forEach> 
							</form:select>
							<form:input style="display:none" path="designerName" htmlEscape="false" class="input-mini required" /> 
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="designerPhone" htmlEscape="false" maxlength="11" class="input-xlarge digits required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="auditorEmployeeId" onchange="changeAuditor()"  style="width: 170px" class="input-xlarge required" >
								<form:option value="" ></form:option>
								<c:forEach items="${auditorList}" var = "auditor">
									<c:if test="${bizPrepareOrder.auditorEmployeeId eq auditor.id }">
										<form:option value="${auditor.id }" selected="selected">${auditor.realname }</form:option>
									</c:if>
									<c:if test="${bizPrepareOrder.auditorEmployeeId ne auditor.id }">
										<form:option value="${auditor.id }" >${auditor.realname }</form:option>
									</c:if>
								</c:forEach> 
							</form:select>
							<form:input style="display:none" path="auditorName" htmlEscape="false" class="input-mini required" /> 
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
						<div class="controls" hidden="hidden">
								<c:forEach items="${auditorList}" var = "auditor">
									<input type="text" value="${auditor.realname }" id="auditorName${auditor.id }">
									<input type="text" value="${auditor.phone }" id="auditorPhone${auditor.id }">
								</c:forEach> 
						</div>
					</div>
				</td>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="auditorPhone" htmlEscape="false" maxlength="11" class="input-xlarge digits required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<input name="contractStartDate" type="text" readonly="readonly" id="startTime" maxlength="20"
								class="input-medium Wdate required" onblur="dateAutoFill()"
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<input name="contractEndDate" type="text" readonly="readonly" id="endTime" onblur="dateAutoFill()" maxlength="20"
								class="input-medium Wdate required"
			...
			<tr>
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="contractArea" htmlEscape="false" maxlength="12" class="input-xlarge required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="contractTime" htmlEscape="false" maxlength="12"
								class="input-xlarge number" id="contractDays"
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<select id="bizOrderAcceptArea" name="bizOrderAcceptArea" style="width: 170px">
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="status" class="input-xlarge required" disabled="true">
								<form:option value="" label="" />
								<form:options items="${fns:getDictList('prepare_order_status')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							</form:select>
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="customerAddress" htmlEscape="false" maxlength="255" class="input-xlarge required" />
							<span class="help-inline"><font color="red">*</font> </span> </br> 
							<input htmlEscape="false" maxlength="255" class="input-xlarge"
								id="mapCoordinate1" name="mapCoordinate1" placeholder="地图搜索"
								value="${bizPrepareOrder.customerAddress}" />
						</div>
					</div>
				</td>
				<td>
		...
		<div class="form-actions">
			<shiro:hasPermission name="bizprepareorder:bizPrepareOrder:edit">
				<input id="btnSubmit" class="btn btn-primary" type="submit" 
					value="保 存" onmousemove="formSubmit()"  />&nbsp;</shiro:hasPermission>
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)" />
		</div>
		<div hidden="hidden">
			<input type="text"  name="mapCoordinate" id="mapCoordinate" value="${bizPrepareOrder.mapCoordinate}" />
		</div>
	</form:form>
	<div id="searchResultPanel" style="border: 1px solid #C0C0C0; width: 300px; height: 200px; display: none;"></div>
	<script type="text/javascript">
		var baiduAK = "mcgKBNHozGudn6mzNG9weAwOayGKu4Zj";
		// 百度地图API功能
		...
		function setPlace() {
			...
			function myFun() {
				...
				$("#mapCoordinate").val(pp.lng + "," + pp.lat);
				
				$("#customerAddress").val( $("#mapCoordinate1").val());
				map.centerAndZoom(pp, 100);
				mkr = new BMap.Marker(pp, {
					enableDragging : true,
					raiseOnDrag : true
				});
				//添加标注
				...
				mkr.addEventListener("dragend", function(e) {
					var posi = e.point.lng + "," + e.point.lat;
					var posi2 = e.point.lat + "," + e.point.lng;
					$("#mapCoordinate").val(posi);
					var url = "http://api.map.baidu.com/geocoder/v2/?ak="
							+ baiduAK + "&callback=renderReverse&location="
							+ posi2 + "&output=json&pois=1"
					$("#location").val(posi);
					var aj = $.ajax({
						url : url,// 跳转到 action    
						data : {},
						...
						success : function(data) {
							if (data.status == "0") {
								$("#mapCoordinate1").val(data.result.formatted_address);
		...
		function fillCity() {
			...
			for (i = 1; i <= aCity.length; i++) {
				if (aCity[i - 1].substring(2, 6) == "0000") {
					document.getElementById("province").options[j] = new Option(
							aCity[i - 1].substring(7, aCity[i - 1].length),
							aCity[i - 1].substring(0, 6));
					j++;
		...
		function setCity(provinceCode) {
			...
			for (i = 1; i <= aCity.length; i++) {
				if (aCity[i - 1].substring(0, 2) == provinceCode
						&& aCity[i - 1].substring(2, 6) != "0000"
						&& aCity[i - 1].substring(4, 6) == "00") {
					document.getElementById("city").options[j] = new Option(
							aCity[i - 1].substring(7, aCity[i - 1].length),
							aCity[i - 1].substring(0, 6));
					j++;
					//alert(aCity[i - 1].substring(7, aCity[i - 1].length));
				}
			}
		...
		function setCounty(cityCode) {
			...
			for (i = 1; i <= aCity.length; i++) {
				if (aCity[i - 1].substring(0, 4) == cityCode
						&& aCity[i - 1].substring(4, 6) != "00") {
					document.getElementById("county").options[j] = new Option(
							aCity[i - 1].substring(7, aCity[i - 1].length),
							aCity[i - 1].substring(0, 6));
					j++;
				}
		...
		function getArea() {
			if (document.getElementById("province").value != "") {
				document.getElementById("area").value = document
						.getElementById("province").options[document
						.getElementById("province").selectedIndex].text;
			} else {
				document.getElementById("area").value = "什么都没有";
			}
			if (document.getElementById("city").value != "") {
				document.getElementById("area1").value = document
						.getElementById("city").options[document
						.getElementById("city").selectedIndex].text;
			} else {
				document.getElementById("area1").value = "什么都没有";
			}
			if (document.getElementById("county").value != "") {
				document.getElementById("area2").value = document
						.getElementById("county").options[document
						.getElementById("county").selectedIndex].text;
			} else {
		...
		function setArea() {
			for (i = 1; i <= document.getElementById("province").length; i++) {
				if (document.getElementById("province").options[i - 1].value
						.substring(0, 2) == document.getElementById("area").value
						.substring(0, 2)) {
			...
			for (i = 1; i <= document.getElementById("city").length; i++) {
				if (document.getElementById("city").options[i - 1].value
						.substring(0, 4) == document.getElementById("area").value
						.substring(0, 4)) {
			...
			for (i = 1; i <= document.getElementById("county").length; i++) {
				if (document.getElementById("county").options[i - 1].value == document
		...
		function selectOption() {
			var selOption = document.getElementById("province");
			selOption.options[0].selected = true;
			setCity(selOption.value);
			getArea();//默认选中黑龙江省
		}
