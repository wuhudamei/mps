...
<script type="text/javascript">
								...
								.validate(
										{
											submitHandler : function(form) {
												var guaranteeMoney = $(
														"#guaranteeMoney")
														.val();
												var guaranteeMoneyBalance = $(
														"#guaranteeMoneyBalance")
														.val();
												var oldGuaranteeMoneyAmount = $(
														"#oldGuaranteeMoneyAmount")
														.val();
												/* if(parseFloat(guaranteeMoneyBalance)-parseFloat(guaranteeMoney)+parseFloat(oldGuaranteeMoneyAmount)>2000){
													alertx("质保金总额不能超过2000！");
												}else */if (parseFloat(guaranteeMoneyBalance)
														- parseFloat(guaranteeMoney)
														+ parseFloat(oldGuaranteeMoneyAmount) < 0) {
													alertx("质保金总额不能小于0！");
												} else {
													loading('正在提交，请稍等...');
													$("#btnSubmit").attr(
	...
	function findGuaranteeMoneyBalanceByEmployeeId() {
		...
		$
				.ajax({
					url : "${ctx}/guarantee/guaranteeManager/findGuaranteeMoneyBalanceByEmployeeId?employeeId="
							+ employeeId,
					type : "post",
					success : function(data) {
						if (null != data && data != '') {
							$("#guaranteeMoneyBalance").val(
									data.guaranteeMoneyBalance);
						} else {
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">质保金使用修改</a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="bizGuaranteeMoneyPaidUsed"
		action="${ctx}/guarantee/guaranteeManager/saveUseGuarantee"
		method="post" class="form-horizontal">
		<form:hidden path="id" value="${bizGuaranteeMoneyPaidUsed.id}" />
		<input type="hidden" name="guaranteeMoneyType"
			value="${bizGuaranteeMoneyPaidUsed.guaranteeMoneyType}" />
		...
		<table>
			...
			<tr>
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<form:select path="orderId" id="orderId"
								class="input-medium needClear" onchange="findOrderInfoById()">
							</form:select>
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
			...
			<tr>
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="usedEmployeeType" id="usedEmployeeType"
								class="input-medium" onchange="findUseGuaranteeEmp()">
								<form:option value="" label="" />
								<form:options
									items="${fns:getDictList('use_guarantee_emp_type')}"
									itemLabel="label" itemValue="value" htmlEscape="false" />
							</form:select>
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="usedEmployeeId" id="usedEmployeeId"
								class="input-medium"
								onchange="findGuaranteeMoneyBalanceByEmployeeId()">
							</form:select>
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="guaranteeMoneyBalance"
								id="guaranteeMoneyBalance" readonly="true" htmlEscape="false"
			...
			<tr>
				<td>
					<div class="control-group">
						<label class="control-label">使用质保金金额：</label> <input type="hidden"
							id="oldGuaranteeMoneyAmount"
							value="${bizGuaranteeMoneyPaidUsed.guaranteeMoneyAmount}" />
						<div class="controls">
							<form:input path="guaranteeMoneyAmount" id="guaranteeMoney"
								style="width:160px" htmlEscape="false" min="0"
								class="input-xlarge number " required="required" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				</td>
				...
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<input name="guaranteeMoneyDateTime" type="text"
								readonly="readonly" maxlength="20" id="guaranteeMoneyDateTime"
								class="input-medium Wdate required"
								value="<fmt:formatDate value="${bizGuaranteeMoneyPaidUsed.guaranteeMoneyDateTime}" pattern="yyyy-MM-dd"/>"
			...
			<tr>
				<td>
					<div class="control-group">
						...
						<div class="controls">
							<form:textarea path="guaranteeMoneyReason" htmlEscape="false"
								rows="4" maxlength="100" class="input-xxlarge "
								id="reportRemarks" name="reportRemarks" />
						</div>
					</div>
				</td>
			</tr>
