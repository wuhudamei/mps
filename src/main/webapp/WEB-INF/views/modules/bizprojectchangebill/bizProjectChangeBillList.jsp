...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bizprojectchangebill/bizProjectChangeBill/list">设计师审核列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizProjectChangeBill" action="${ctx}/bizprojectchangebill/bizProjectChangeBill/designerList2" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
		<input type="hidden" value="${storeIdhh }" id="storeIdHide"/>
			 <li><label>门店：</label>
			...
			<li><label>状态 ：</label>
				<form:select path="status" class="input-medium needClear">
					<form:option value="" label=""/>
					<form:option value="10" label="">项目经理已提报</form:option>
					<form:option value="15" label="">设计师审核不通过</form:option>
...
<!-- 					</a> -->
			...
			<a href="#" data-toggle="modal" data-target="#myModal-photo"   onclick="ajaxGetImgs ('${bizProjectChangeBill.projectChangeId}','105')">查看</a>
				...
				<shiro:hasPermission name="bizprojectchangebill:bizProjectChangeBill:edit"><td>
    				...
    				<c:if test="${bizProjectChangeBill.status=='10'}">
	...
	<script type="text/javascript">
		...
		function yesReject(){
			...
			if(reason!=null && reason!=""){
				window.location.href = "${ctx}/bizprojectchangebill/bizProjectChangeBill/refusePass?projectChangeId="+projectChangeId+"&reason="+reason+"&status=10";
			}else{
				alert("驳回原因不能为空");
			}
		}
