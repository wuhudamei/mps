...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/evaluate/bizEvalWorkGrade/openBizEvalWorkGradePage">工人评分查询</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizEvalWorkGrade" action="${ctx}/evaluate/bizEvalWorkGrade/queryBizEvalWorkGrade" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>门店：</label>
				<c:if test="${empty storeDropEnable}">
					<form:select id="storeId" path="storeId" class="input-medium" disabled="true" onchange="findEngineListByStoreIdAndProjectMode()">
			...
			<li><label>工程模式：</label>
				<c:if test="${empty gongcheng}">
					<form:select id="projectMode" path="projectMode" class="input-medium" disabled="true" onchange="findEngineListByStoreIdAndProjectMode()">
			...
			<li><label>工人组长：</label>
				<form:input path="groupRealName" htmlEscape="false" maxlength="64" class="input-medium"/>
			</li>
			...
			<li><label>任务包名称：</label>
				<form:input path="packageName" htmlEscape="false" maxlength="64" class="input-medium"/>
			</li>
			
			<li><label style="width: 120px">评价时间：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizEvalWorkGrade">
			<tr>
				<td>
					${fns:getStoreLabel(bizEvalWorkGrade.storeId, '')}
				</td>
				<td>
					${fns:getDictLabel(bizEvalWorkGrade.projectMode, 'project_mode', '')}
				</td>
