...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bizorderinstallitemproblem/bizOrderInstallItemProblem/listVo">订单安装项问题列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizOrderInstallItemProblemVo" action="${ctx}/bizorderinstallitemproblem/bizOrderInstallItemProblem/listVoLast" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>门店：</label>
				<c:if test="${empty storeDropEnable}">
					<form:select path="storeId" class="input-medium" disabled="true" onchange="queryInstallItemProblemType()">
						<form:options items="${fns:getStoreList()}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</c:if>
				<c:if test="${!empty storeDropEnable}">
					<form:select path="storeId" class="input-medium needClear" onchange="queryInstallItemProblemType()">
						<form:options items="${fns:getStoreList()}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</c:if>
			</li>
			<li><label>工程模式：</label>
				<c:if test="${empty gongcheng}">
					<form:select path="projectMode" class="input-medium" disabled="true" onchange="queryInstallItemProblemType()">
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('project_mode')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</c:if>
				<c:if test="${!empty gongcheng}">
					<form:select path="projectMode" class="input-medium needClear" onchange="queryInstallItemProblemType()">
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('project_mode')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</c:if>
			...
			<li><label>状态：</label>
				<form:select path="status" class="input-medium needClear">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('install_item_problem_status')}"   itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>客户姓名：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizOrderInstallItemProblem">
			<tr>
				...
				<td>
					<a href="${ctx}/bizorderinstallitemproblem/bizOrderInstallItemProblem/photo?id=${bizOrderInstallItemProblem.id}">${bizOrderInstallItemProblem.picturesCount}张</a>
				</td>
				<td>
					<c:if test="${bizOrderInstallItemProblem.status == '10'}">
						<a href="#" onclick="showDeal(this,'${bizOrderInstallItemProblem.id}','30')">通过</a>
						<a href="#" onclick="showDeal(this,'${bizOrderInstallItemProblem.id}','35')">驳回</a>
					</c:if>
		...
		<form id="urgeForm">
			<div class="tc_center">
	            ...
	            <div class="cen_t">
	                ...
	                <div class="cen_tex">
						<span class="span_l" id="problemSolveNotesTitle">内容：</span>
						<p class="span_r">
	...
	<script type="text/javascript">
	    ...
	    //工程安装问题上报审核
		function  showDeal(obj,problemId,status){
			
	  		problemIdGlobal=problemId;
	  		problemStatusGlobal=status;
			...
			if(status=="30"){
				$("#problemSolveNotesTitle").text("说明：");
			}else{
				$("#problemSolveNotesTitle").text("驳回理由：");
			}
		...
		function yes(){
			...
			if(null==problemSolveNotes || problemSolveNotes==""){
				$("#alertRemarks").text("请输入内容");
           		$("#subReport").show();
				return false;
			}
			
			$.ajax({
				url: "${ctx}/bizorderinstallitemproblem/bizOrderInstallItemProblem/update_install_problem_ajax",
	            type: "post",
	            data:{
	            		problemId:problemIdGlobal,
	            		problemSolveNotes:problemSolveNotes,
	            		status:problemStatusGlobal
	            	},
	            success: function(data) {
	            	...
	            	if(null!=data && data=="0"){
	            		...
	            		$("#alertRemarks").text("问题上报处理内容为空");
	            		$("#subReport").show();
	            	}else if(data=="3"){
	            		$("#alertRemarks").text("问题上报处理状态为空");
	            		$("#subReport").show();
	            	}else if(data=="4"){
	            		$("#alertRemarks").text("该问题已审核");
	            		$("#subReport").show();
	            	}else if(data=="5"){
