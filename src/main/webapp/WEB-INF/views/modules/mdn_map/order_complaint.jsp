...
<html>
	<head>
	   ...
	   <%-- <script type="text/javascript" src="http://webapi.amap.com/maps?v=1.3&key=eea8244dadd58de30dd93b36ca8f0f90"></script>--%>
	    ...
	    <script type="text/javascript">
			...
			$(function(){
			    $("#status option[value='20']").attr("selected","selected");
	...
	<c:set var="user" value="${fns:getUser()}"></c:set>
		<ul class="nav nav-tabs">
			<li class="active"><a href="${ctx}/ordercomplan/bizOrderComplaint/mapList">订单投诉地图中心</a></li>
		</ul>
<form:form id="searchForm" modelAttribute="bizOrderComplaint"  method="post" class="breadcrumb form-search">
			<ul class="ul-form">
				<li><label>门店：</label>
				...
				<li class="btns" style="width: auto;"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询" onclick="chaxun()"/></li>
				<li class="btns" style="width: auto;"><input class="btn btn-primary clearBtn" type="button" value="清空" onclick="clearCondition()"/></li>
		...
		<script type="text/javascript">
			...
			function chaxun(){
				...
				if(!storeId){
					...
					loading("正在查询...请稍等")
					var options ={
							url: "${ctx}/ordercomplan/bizOrderComplaint/mapListInfo",
							type: "post",
							success:function(data){
								...
								if(data!=null&& data.length>0){
                                    ...
                                    var myIcon = new BMap.Icon("${ctxStatic}/modules/complaint/icon.png", new BMap.Size(50,27));
                                        ...
                                        for (var v = 0; v < data.length; v++) {
                                            ...
                                            //返回一个对象
                                            if(data[v].pointx!="" &&data[v].pointy!=undefined&&data[v].pointy!=null&&data[v].pointx!="" &&data[v].pointx!=null&&data[v].pointx!=undefined){

                                                var marker = new BMap.Marker(new BMap.Point(data[v].pointx,data[v].pointy),{icon:myIcon});
                                                var content =("地址:"+data[v].detailAddress+"<br>客户:"+data[v].customerName+"<br>电话:"+data[v].customerPhone+"<br>项目经理:"+data[v].itemManager+"<br>手机号:"+data[v].itemManagerIphnoe+"<br>-------------------------------------<br>投 诉 问 题 :<br>"+data[v].bizOrderComplaintProblem.complaintProblemName);
