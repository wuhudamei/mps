...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bizorderrecheck/bizOrderRecheck/preList">项目经理复尺列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="bizOrderRecheck" action="${ctx}/bizorderrecheck/bizOrderRecheck/list" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>申请复尺日期：</label>
				<input name="appRecheckStart" type="text" id="appRecheckStart" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizOrderRecheck.appRecheckStart}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'appRecheckEnd\')}',isShowClear:false});"/> &nbsp;至&nbsp;
					
				<input name="appRecheckEnd" type="text" id="appRecheckEnd" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizOrderRecheck.appRecheckEnd}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'appRecheckStart\')}',isShowClear:false});"/>
			</li>
			<li><label>复尺项目：</label>
...
<%-- 						<a href="${ctx}/bizbusinesspic/bizBusinessPic/showRecheckPic?recheckID=${p.recheckID}&type=201"> --%>
<!-- 							查看 -->
<!-- 						</a> -->
...
<!-- 						</a> -->
				...
				<shiro:hasPermission name="bizorderrecheck:bizOrderRecheck:view"><td>
    				...
    				<a href="${ctx}/bizorderrecheck/bizOrderRecheck/recheckDetail?recheckID=${p.recheckID}&type=${p.recheckType }&orderID=${p.id }">详情</a>
