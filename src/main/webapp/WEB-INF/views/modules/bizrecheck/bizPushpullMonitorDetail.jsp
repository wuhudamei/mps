...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">推拉门复尺详情</a></li>
	</ul>
	...
	<div class="container">
		...
		<table class="table table-striped table-bordered table-condensed">
			...
			<tbody>
				<c:forEach items="${listPull }" var="pull">
					<tr><td>${fns:getDictLabel(pull.position, 'recheck_pushdoor_position', '')}</td>
					<td>${fns:getDictLabel(pull.pushPullStyle, 'recheck_pushdoor_pushpullstyle', '')}</td>
					<td>${fns:getDictLabel(pull.packageCover, 'recheck_pushdoor_packagecover', '')}</td>
					<td>${fns:getDictLabel(pull.openDirection, 'recheck_pushdoor_opendirection', '')}</td>
					<td>${pull.doorAmount }个</td>
					<td><fmt:formatNumber value="${pull.newDoorWidth }" pattern="#"/>mm</td>
					<td><fmt:formatNumber value="${pull.newDoorHeight }" pattern="#"/>mm</td>
					<td><fmt:formatNumber value="${pull.width }" pattern="#"/>mm</td>
					<td><fmt:formatNumber value="${pull.height }" pattern="#"/>mm</td>
					<td><fmt:formatNumber value="${pull.thickness }" pattern="#"/>mm</td></tr>
				</c:forEach>
			</tbody>
		</table>
	</div>
