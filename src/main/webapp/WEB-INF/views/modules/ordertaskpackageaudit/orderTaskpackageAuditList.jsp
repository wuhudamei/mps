...
<head>
	...
	<script type="text/javascript">
		...
		function searchButton(){
			...
			var engineDepartId = $("#enginDepartId").val();
			if(storeId == ""){
				alertx("请选择门店+任务包状态才能进行查询。"); 
				return false;
			}
			...
			$("#searchForm").attr("action", "${ctx}/ordertaskpackageaudit/orderTaskpackageAudit/list");
            $("#searchForm").submit();
		}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/ordertaskpackageaudit/orderTaskpackageAudit/preList">已审核任务包列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="orderTaskpackageAudit" action="" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li>
				<label>任务包状态：</label>
				<input type="checkbox" name="chkAll" id="chkAll" onclick="ChkAllClick('packageStateids','chkAll')" />全部
				<form:checkboxes path="packageStateids" id="packageStateids" items="${packageStateids}" itemLabel="label" itemValue="value" htmlEscape="false" class="required"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="orderTaskpackageAudit" varStatus="index">
			<tr id="trId${index.index }">
				...
				<td>
					${fns:getDictLabel(orderTaskpackageAudit.packageStateid, 'taskpackage_status', '')}
				</td>
				<td>
					<fmt:formatDate value="${orderTaskpackageAudit.packAuditTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					${orderTaskpackageAudit.packAuditName}
				</td>
				<shiro:hasPermission name="ordertaskpackageaudit:orderTaskpackageAudit:edit"><td>
					<c:if test="${orderTaskpackageAudit.packageStateid == '10'}">
						<a href="${ctx}/ordertaskpackageaudit/orderTaskpackageAudit/showView?orderId=${orderTaskpackageAudit.orderId}&packCode=${orderTaskpackageAudit.packageCode}&id=${orderTaskpackageAudit.id}&flag=1">查看详情</a>
	   				</c:if>
	   				<c:if test="${orderTaskpackageAudit.packageStateid == '10'}">
	   				...
	   				<c:if test="${orderTaskpackageAudit.packageStateid == '10' || orderTaskpackageAudit.packageStateid == '20' || orderTaskpackageAudit.packageStateid == '50'
						...
						&packCode=${orderTaskpackageAudit.packageCode}&taskpackageId=${orderTaskpackageAudit.id}&flag=1">修改</a>
	   				</c:if>
