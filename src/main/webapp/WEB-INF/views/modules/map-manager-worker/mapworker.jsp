...
<html>
	...
	<c:set var="user" value="${fns:getUser()}"></c:set>
		<ul class="nav nav-tabs">
			<li class="active"><a href="${ctx}/order2/order2/workerMapCenterPre">产业工人组指挥中心</a></li>
		</ul>
		<form:form id="searchForm" method="post" modelAttribute="order2" class="breadcrumb form-search">
			...
			<li><label>区域：</label>
				...
				<li><label>工人组长：</label>
					<form:input path="workeMaprName" htmlEscape="false" maxlength="11" class="input-medium"/>
				</li>
			
				<li><label>常用工种：</label>
				<form:select path="workerMapType" class="input-medium needClear">
				<form:option value="" label="" />
		...
		<script type="text/javascript">
			...
			function chaxun(){
				...
				if(!storeId){
                    ...
                    loading("正在查询...请稍等")
					var options ={
							url: "${ctx}/order2/order2/workerMapCenter",
							type: "post",
							success:function(data){
								...
								if(data!=null&& data.length>0){
                                    ...
                                    for (var v = 0; v < data.length; v++) {
                                        ...
                                        var content=("组长:"+data[v].workeMaprName+"<br>星级:"+data[v].starLevel+"星<br>手机:"+data[v].workerMapPhone+"<br>地址"+data[v].workerMapAddress);
