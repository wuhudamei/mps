...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/ordertaskpackagepaymentsummary/bizOrderTaskpackagePaymentSummary/summaryAllLoadList">批次对应付款单明细列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizOrderTaskpackagePaymentSummary" action="${ctx}/ordertaskpackagepaymentsummary/bizOrderTaskpackagePaymentSummary/summaryAllLoadList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="summary">
			<tr>
				...
				<td>${fns:getSummaryStatus(summary.status)}</td>
				<td><fmt:formatDate value="${summary.generatedDatetime}" pattern="yyyy-MM-dd"/></td>
				<td>${summary.orderTaskpackagePaymentCode}</td>
				<td>${fns:getOrderTaskpackagePaymentStatus(summary.paymentStatus)}</td>
				<td><fmt:formatDate value="${summary.paymentGeneratedDatetime}" pattern="yyyy-MM-dd"/></td>
				<td>${summary.paymentAmount}</td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
