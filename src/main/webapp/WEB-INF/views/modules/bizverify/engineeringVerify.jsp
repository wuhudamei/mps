...
<script type="text/javascript">
	...
	function yesReject(){
		...
		$("#reason").val("");
		$("#refuseSalary").addClass("undis");
		window.location.href = "${ctx}/verify/bizAuxiliaryMaterialsVerify/updateVerifyStatus?type=2&verifyId="+verifyId+"&status=25&cancelReason="+reason;
	}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="${ctx}/verify/bizAuxiliaryMaterialsVerify/openEngineeringVerifyPage">工程部对账单审核</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizAuxiliaryMaterialsVerify"
		action="${ctx}/verify/bizAuxiliaryMaterialsVerify/engineeringVerifyList" method="post"
		...
		<ul class="ul-form">
            ...
            <form:select
					...
					<form:option value="20" label="供应商对账单申请" />
					<form:option value="25" label="工程部对账单驳回" />
		...
		<tbody>
			<c:forEach items="${page.list}" var="verify">
				<tr>
					...
					<td>
					    <c:if test="${verify.status == '20'}">
					     <a href="${ctx}/verify/bizAuxiliaryMaterialsVerify/updateVerifyStatus?type=2&verifyId=${verify.id}&status=30" onclick="return confirmx('确认要审核通过吗？', this.href)">通过</a>
					     <a href="#" onclick="reject(${verify.id})">驳回</a>
					    </c:if>
						<a href="${ctx}/verify/bizAuxiliaryMaterialsVerify/verifyAuxiliaryDetail?auxiliaryMaterialsVerifyId=${verify.id}&groupType=2">详情</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
