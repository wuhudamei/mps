	...
	<script type="text/javascript">
		...
		function save(id,storeId,communityName,buildNumber,buildUnit,buildRoom,designerName,designerPhone,customerName,customerPhone){
		    ...
		    if(size == 1){	
		    	//重新分配
		    	if(1==isReDistribute){
		    		window.location.href = "${ctx}/order2/order2/resendManager1?id="+id+"&managerName="+employeeName+"&managerId="+employeeId+"&managerPhone="+managerPhone+"&tradition="+1;
		    	}else{
		    		window.location.href = "${ctx}/order2/order2/saveManager1?id="+id+"&managerName="+employeeName+"&managerId="+employeeId+"&managerPhone="+managerPhone+"&tradition="+1;
		    	}
		    }else{
		    	alert("请选择项目经理！");
		    	return;
		...
		function fanhui(){
			...
			if(hidProjectMode == "1"){
				...
				window.location.href = "${ctx}/order2/order2/listUnAllotZCY";
			}
		}
