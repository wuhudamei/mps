...
<html>
	...
	<body>
		<ul class="nav nav-tabs">
			<li class="active"><a href="">地图重新分配项目经理</a></li>
		</ul>
		<form:form id="searchForm" modelAttribute="order"  method="post" class="breadcrumb form-search">
			<ul class="ul-form">
				...
				<li><input id="btnSubmit" class="btn btn-primary" name="btnSubmit" type="button" value="查询" onclick="getItemManager()" /></li>
				<li><input class="btn btn-primary clearBtn" type="button" value="清空" onclick="clearCondition()"/></li>
				<li><input class="btn btn-primary" type="button" value="返回" onclick="window.location.href='${ctx}/order2/order2/reAllotManager?id=${order.orderId}'"/></li>
				<li class="clearfix"></li>
			</ul>
			</form:form>
		...
		//页面加载 发送ajax 查出工人组marker
			window.onload = function() {
				loading("查询中,请稍等...");
				$.ajax({
					url : "${ctx}/order2/order2/managerAllotMap?orderId=${order.orderId}",
					success : function(data) {
						...
						for (var v = 0; v < data.length; v++) {
							...
							var title = '项目经理信息<input type="button" onclick="showConfirm('+data[v].id + ',  \''+ data[v].realname +'\','+data[v].phone+','+${order.orderId}+')" value="重新分配项目经理" style="height:30px;width:120px;color:blue;"></input>'

                            var content =("<img src='${ctxStatic}/modules/info.png'><p style='margin:buttom:3px;'>姓名: "+data[v].realname+"</p><p style='margin:buttom:3px;'>星级: "+data[v].star+"星</p><p style='margin:buttom:3px;'>承接量: "+data[v].alreadyDistributeCount+"/"+data[v].doNow+"/"+data[v].totalCount+"</p><p style='margin:buttom:3px;'>距离："+data[v].distance+"</p>");
		...
		//包括 : 元素内容,标签,节点,事件,样式 等
		    ...
		    function closeInfoWindow() {
				...
				function getItemManager(){
					...
					$.ajax({
						url : "${ctx}/order2/order2/managerAllotMap?orderId=${order.orderId}"+"&distance="+distance+"&star="+star,
						success : function(data) {
				...
				function share(id,realname,phone,orderId){
					...
					console.log("项目经理id"+id+"项目经理"+realname+"电话"+phone+"订单编号"+orderId);
					
					
					if(null!=id && id!=""){
						window.location.href = "${ctx}/order2/order2/resendManager?id="+orderId+"&managerName="+realname+"&managerId="+id+"&managerPhone="+phone;
					}
				}
				//取消按钮
				function quxiao(){
