...
<script type="text/javascript"
	src="http://api.map.baidu.com/api?v=2.0&ak=mcgKBNHozGudn6mzNG9weAwOayGKu4Zj"></script>
<script type="text/javascript">
	$(document).ready(function() {
		$("#btnSubmit").click(function(){
			if($("#mapCoordinate1").val() !=null && $("#mapCoordinate1").val() != ""){
				if($("#mapCoordinate").val()==null || $("#mapCoordinate").val()==""){
					$("#mapCoordinateId").html("请选择订单的地图地址*");	
					return false;
				}else{
					$("#mapCoordinateId").html("*");	
				}	
			}
		    ...
		    return format.replace(/yyyy|MM|dd|HH|mm|ss/g, function(a){
				...
				var readOnly = "${readOnly}"
					...
					if ("" == projectMode) {
						...
						$("#acceptOrderDateId").val(format(now,'yyyy-MM-dd'))	
...
$("#inputForm")
						.validate(
								{
									submitHandler : function(form) {
										var flag = checkDesignerAndAuditor();
										if(flag){
											loading('正在提交，请稍等...');
	...
	//校验设计师和审计员
	function checkDesignerAndAuditor(){
		
		//设计师校验
		var designerEmployeeId = $("#designerEmployeeId").val();
		var designerName = $("#designerName").val();
		var designerPhone = $("#designerPhone").val();
		var designerNameStandard = $("#designerName"+designerEmployeeId).val();
		var designerPhoneStandard = $("#designerPhone"+designerEmployeeId).val()
		if(designerName != designerNameStandard || designerPhone!=designerPhoneStandard){
 ...
 function  FloorAutoWrite() {
	...
	function checkIsSame() {
		var inputOrderNumberValue = $("#orderNumber").val();
		if (inputOrderNumberValue == null || inputOrderNumberValue == "") {
			alertx("请输入订单编号");
			return;
		}
		$.ajax({
			url : "${ctx}/order/order/checkOrderNumber?orderNumber=" + inputOrderNumberValue,
			type : "get",
	...
	function formSubmit() {
		...
		if (item.length > 0) {
			for (var v = 0; v < item.length; v++) {
				if (item[v].checked) {
					$(item[v]).attr("name", "installItemIds");
				}
	...
	var html = "";
		var storeId = $("#sel").val();
		var projectModeValue = $(
				":radio[name='projectMode'][checked='checked']").val();
		if (storeId < 1) {

			return;
		}
		html += '<td><span>订单相关安装项</span></td>';
		//根据门店,动态加载安装项
		$
				.ajax({

					url : "${ctx}/order/order/findInstallItemByStoreId?storeId="
					...
					success : function(data) {
						if (data.length > 0) {
							...
							for (var v = 0; v < data.length; v++) {
								html += '<td><label>'
										+ data[v].projectInstallItemName
										+ '</label><div class="checkboxFive"><input class= "installItemClass" type="checkbox" value="'+data[v].projectInstallItemId+'"/></td>';
							}
							$("#installTRID").html('<td></td>');
							$("#installTRID td").html(html);
							
							if(projectModeValue==2){
								$(".installItemClass").attr("checked",true);
							...
							html += '<label>该门店暂无安装项</label>';
							$("#installTRID").html(html);
						}
	...
	function changeCheck(){
		var designerEmployeeId = $("#designerEmployeeId").val();
		$("#designerName").val($("#designerName"+designerEmployeeId).val());
		$("#designerPhone").val($("#designerPhone"+designerEmployeeId).val());
	}
...
function 	findAcceptArea(){
	
	$("#s2id_acceptArea").each(function(){
		$(this).find("span[class=select2-chosen]").html('---请选择地区---');
		
	})
	var html2 = '<option value="" selected="selected">---请选择地区---</option>';
	...
	$.ajax({
				...
				success : function(data) {
					if (data.length > 0) {
						...
						$("#acceptArea").html(html2);
					} else {
						html2 += '<option value="" selected="selected">---请选择地区---</option>';
						$("#acceptArea").html(html2);
					}
	...
	$.ajax({
				...
				type : "get",
				success : function(data) {
					var engineDepartObj =document.getElementsByName("engineDepartId");
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a
			href="${ctx}/order/order/formUser?id=${order.id}">订单<shiro:hasPermission
					name="order:order:edit1">${not empty order.id?'修改':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="order:order:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="order"
		action="${ctx}/order/order/saveUser" method="post" class="form-horizontal">
		<form:hidden path="id" />
		...
		<table>
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="storeId" class="input-medium required"
								id="sel" onchange="findInstallItem()">
			...
			<tr>
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input   readonly="true"  style="border: 1px solid #DDD; background-color: #F5F5F5; color:#ACA899;"  path="orderNumber" htmlEscape="false" maxlength="100"
								class="input-xlarge required" id="orderNumber"
								onblur="checkIsSame()" />
							<span class="help-inline"><font color="red">*</font> </span> <span
								hidden="hidden" id="error"><font color="red">订单编号已重复,请重新输入</font>
							</span>
						</div>
					</div></td>
				<td><div class="control-group">
	...
	<tr>
				...
				<div class="control-group">
						...
						<div class="controls">
							<select name="engineDepartId" style="width: 170px" class = "required" id="engineDepartObj">
							<c:if test="${empty order.id  }">
								<option value=""></option>
							</c:if>
								<c:forEach items="${engineList }" var="engine">
									<option value="${engine.engineDepartId }" <c:if test="${engine.engineDepartId == order.engineDepartId }">selected="selected"</c:if>>${engine.engineDepartName }</option>
								</c:forEach>
								</select>
					<span class="help-inline"><font color="red">*</font> </span>
						</div>
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<input name="acceptOrderDate" type="text" readonly="readonly"
								 maxlength="20" id="acceptOrderDateId"
			...
			<tr>
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input   readonly="true"  style="border: 1px solid #DDD; background-color: #F5F5F5; color:#ACA899;"  path="customerName" htmlEscape="false" maxlength="16"
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input  readonly="true"  style="border: 1px solid #DDD; background-color: #F5F5F5; color:#ACA899;"  path="customerPhone" htmlEscape="false"
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="distanceFee" htmlEscape="false" maxlength="100"
								class="input-medium number" onkeyup="fix_amountthis(this)" onafterpaste="fix_amountthis(this)"/>
						</div>
					</div>
			...
			<tr>
				<td>
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="buildRoom" htmlEscape="false" maxlength="6"
								class="input-mini required" onchange="FloorAutoWrite()" />
							<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div>
				<div class="control-group">
						...
						<div class="controls">
							<form:input path="floor" htmlEscape="false" maxlength="4"
										class="input-mini required" onkeyup="checkNum(this)"
					...
					<div class="control-group">
						...
						<div class="controls">
							<select id="province" onchange="setCity(this.value);getArea();"
								name="province" style="width: 170px" runat="server"
								class="required">
								<c:if test="${order.province!=null }">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select id="city" onchange="setCounty(this.value);getArea();"
								...
								<c:if test="${order.city!=null }">
									<option value="${order.city }" selected="selected">${order.city }</option>
								</c:if>
								<c:if test="${order.city==null }">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select id="county" name="county" style="width: 170px"
								...
								<c:if test="${order.county!=null }">
									<option value="${order.county }" selected="selected">${order.county }</option>
								</c:if>
								<c:if test="${order.county==null }">
									<option value="" selected="selected"></option>
								</c:if>
							</select>
							<!-- <input type="hidden" name="area" id="area" value="什么都没有" /> <input
								type="hidden" name="area1" id="area1" value="什么都没有" /> <input
								type="hidden" name="area2" id="area2" value="什么都没有" /> <span
			...
			<tr>
				<td><div class="control-group">
						...
						<div class="controls">
							<form:select path="designerEmployeeId" onchange="changeCheck()"  style="width: 170px" class="input-xlarge required" >
								<form:option value="" ></form:option>
								<c:forEach items="${empList}" var = "employee">
									<c:if test="${order.designerName eq employee.realname }">
										<form:option value="${employee.id }" selected="selected">${employee.realname }</form:option>
									</c:if>
						...
						<div class="controls" hidden="hidden">
								<c:forEach items="${empList}" var = "designer">
									<input type="text" value="${designer.realname }" id="designerName${designer.id }">
									<input type="text" value="${designer.phone }" id="designerPhone${designer.id }">
								</c:forEach> 
						</div>
					</div></td>
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input path="designerPhone" htmlEscape="false"
								maxlength="11" class="input-xlarge digits required" />
								<span class="help-inline"><font color="red">*</font> </span>
						</div>
					</div></td>
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input path="orderReporterName" htmlEscape="false"
								maxlength="12" class="input-xlarge " />
						</div>
					</div></td>
				...
				<td><div class="control-group">
						...
						<div class="controls">
							<input name="contractEndDate" type="text" readonly="readonly"
								id="endTime" onblur="dateAutoFill()" maxlength="20"
								class="input-medium Wdate required"
			...
			<tr>
				<td><div class="control-group">
						...
						<div class="controls">
							<form:input path="customerAddress" htmlEscape="false"
							...
							<span class="help-inline"><font color="red" id="mapCoordinateId">*</font> </span>
						</div>
					</div></td>
				...
				<td>
					...
					<div class="control-group">
						<label class="control-label">接单范围：</label>
						<div class="controls">
							<select id="acceptArea" name="acceptAreaId" style="width: 170px">
								<c:if test="${order.acceptArea!=null }">
									<option value="${order.acceptAreaId }" selected="selected">${order.acceptArea }</option>
								</c:if>
								<c:if test="${order.acceptArea==null }">
								...
								<c:forEach items="${acceptAreaList }" var="acceptArea">
									<option value="${acceptArea.acceptAreaId }">${acceptArea.acceptArea }</option>
			...
			<tr>
				<td>
					<div id="allmap"
						style="border: 1px solid #C0C0C0; width: 600px; height: 400px;" />
				</td>
				<td></td>
			</tr>
...
<%-- 								<td><label>${item.projectInstallItemName }</label> --%>
<!-- 								<div class="checkboxFive"> -->
<%-- 										<input type="checkbox" value="${item.projectInstallItemId }" --%>
<!-- 											id="checkboxFiveInput" checked="checked" -->
...
<!-- 								<div class="checkboxFive"> -->
<%-- 										<input type="checkbox" value="${item.projectInstallItemId }" --%>
<!-- 											id="checkboxFiveInput" checked="checked" /></td> -->
<%-- 							</c:if> --%>
...
<!-- 			</tr> -->
		...
		<div hidden="hidden">
		<input type="text"  name="mapCoordinate" id="mapCoordinate" value="${order.mapCoordinate}" />
		</div>
	</form:form>
	<div id="searchResultPanel"
		style="border: 1px solid #C0C0C0; width: 300px; height: 200px; display: none;"></div>
	<script type="text/javascript">
					...
					var aj = $.ajax({
						...
						success : function(data) {
							...
							if (data.status == "0") {
                             ...
                             $("#mapCoordinate").val(data.result.location.lng+ "," +data.result.location.lat);
								$("#customerAddress").val($("#mapCoordinate1").val());
