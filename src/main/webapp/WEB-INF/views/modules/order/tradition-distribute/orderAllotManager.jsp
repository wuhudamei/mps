	...
	<script type="text/javascript">
		...
		function save(id,storeId,communityName,buildNumber,buildUnit,buildRoom,designerName,designerPhone,customerName,customerPhone){
		    ...
		    if(size == 1){	
		    	...
		    	//重新分配
		    	if(1==isReDistribute){
		    		window.location.href = "${ctx}/order2/order2/resendManager?id="+id+"&managerName="+employeeName+"&managerId="+employeeId+"&managerPhone="+managerPhone+"&tradition="+1;
				    	
		    		
		    		
		    	}else{
		    		
		    	window.location.href = "${ctx}/order2/order2/saveManager?id="+id+"&managerName="+employeeName+"&managerId="+employeeId+"&managerPhone="+managerPhone+"&tradition="+1;
		...
		//返回
		function fanhui(){
			window.location.href = "${ctx}/order2/order2/listAllotManagerTradition";
		}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/order2/order2/listAllotManagerTradition?id=${order.id}">分配项目经理</a></li>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="itemManager" action="${ctx}/order2/order2/allotManagerTradition?id=${order.id}" method="post" class="breadcrumb form-search">
