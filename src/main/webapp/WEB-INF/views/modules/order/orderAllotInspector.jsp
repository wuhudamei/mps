	...
	<script type="text/javascript">
		...
		function save(id,communityName,buildNumber,buildUnit,buildRoom,customerName,customerPhone){
		    ...
		    if(size == 1){	
			    ...
			    window.location.href = "${ctx}/order2/order2/saveInspector?id="+id+"&inspectorName="+employeeName+"&inspectorId="+employeeId+"&inspectorPhone="+inspectorPhone+"&isLongwayCommission="+isLongwayCommission;
		...
		function mapInspector(id){
			window.location.href = "${ctx}/order2/order2/mapInspector?orderId="+id;
		}
		...
		function fanhui(){
			...
			if(hidProjectMode == "1"){
				window.location.href = "${ctx}/order2/order2/listUnAllotInspector";
			}else if(hidProjectMode == "2"){
				window.location.href = "${ctx}/order2/order2/listUnAllotInspectorTradition";
			}else{
				window.location.href = "${ctx}/order2/order2/quasiIndustryDfpzjy";
			}
		}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/order2/order2/allotInspector?id=${order.id}">分配质检员</a></li>
	</ul>
		...
		<table>
			...
			<tr>
				<td colspan="3">
					...
					<input id="map" class="btn btn-primary" name="btnSubmit" type="button" value="地图分配质检员" onclick="mapInspector('${order.id}')"/>
				</td>
			</tr>
		</table>
	</form:form>
	...
	<form:form id="searchForm" modelAttribute="inspector" action="${ctx}/order2/order2/allotInspector?id=${order.id}" method="post" class="breadcrumb form-search">
