...
<html>
	...
	<body>
		<ul class="nav nav-tabs">
			<li class="active"><a href="">地图重新分配质检员</a></li>
		</ul>
		<form:form id="searchForm" modelAttribute="order"  method="post" class="breadcrumb form-search">
			<ul class="ul-form">
				...
				<li><input class="btn btn-primary" type="button" value="返回" onclick="window.location.href='${ctx}/order2/order2/reAllotInspector?id=${order.orderId}'"/></li>
				<li class="clearfix"></li>
			</ul>
			</form:form>
				...
				$.ajax({
					...
					success : function(data) {
						...
						for (var v = 0; v < data.length; v++) {
							...
							var title = '质检员信息<input type="button" onclick="showConfirm('+data[v].id + ',  \''+ data[v].realname +'\','+data[v].phone+','+${order.orderId}+')" value="重新分配质检员" style="height:30px;width:120px;color:blue;"></input>'
		...
		//包括 : 元素内容,标签,节点,事件,样式 等
		    ...
		    function closeInfoWindow() {
				...
				function share(id,realname,phone,orderId){
					...
					if(null!=id && id!=""){
						window.location.href = "${ctx}/order2/order2/resendInspector?id="+orderId+"&inspectorName="+realname+"&inspectorId="+id+"&inspectorPhone="+phone;
					}
				}
				//取消按钮
				function quxiao(){
