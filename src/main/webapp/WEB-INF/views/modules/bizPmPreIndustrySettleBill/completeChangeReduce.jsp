...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="javascript:void(0)">竣工变更减项</a></li>
	</ul>
	...
	<div class="container">
		<div class="row clearfix">
			...
			<div class="col-md-12 column">
				<form id="inputSubmit"
					action="${ctx}/proIndustryPmSettle/proIndustryPmSettle/saveBaseInstalled">
					<input type="hidden" name="id" value="${bizOrderChange.id}" /> <input
						type="hidden" name="orderId" value="${bizOrderChange.orderId}" />
					<input type="hidden" name="changeType"
						value="${bizOrderChange.changeType}" />
					<input type="hidden" name="isEditSettleBill" value="${isEditSettleBill}"/>
					<table class="table table-striped table-bordered table-condensed">
						<tr>
							<td>竣工变更减项金额：<input type="text" id="changeAmount" readonly="readonly"
								name="changeAmount" value="${bizOrderChange.changeAmount}"
								oninput="this.value=this.value.replace(/[^\d\.]/g,'');initMoney()">
								元
							</td>
						</tr>
						<input type="hidden" name="changeAccountRate"
							id="changeAccountRate" value="100" />
							
						<input type="hidden" name="changeAccountAmount"
							id="changeAccountAmount" readonly="readonly"
							value="${bizOrderChange.changeAccountAmount}" />
						<tr>
							<td>竣工变更减项结算说明：<textarea name="changeRemarks" readonly="readonly">${bizOrderChange.changeRemarks}</textarea></td>
						</tr>
					</table>
				</form>
			</div>
