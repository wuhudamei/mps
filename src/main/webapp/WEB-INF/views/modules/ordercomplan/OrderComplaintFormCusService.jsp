...
<head>
    ...
    <style>
        ...
        .twoBtns .maskBtn:first-child {
            background: #0780ec;
            border-radius: .1rem;
            float: left;
        }

        .twoBtns .maskBtn:last-child {
...
// 			$(this).parent().remove();
        ...
        $(document).on('click', '.addBtn', function () {
            ...
            var itemList = '<div class="mb1d mb12 rela itemList">' +
                ...
                '</select>' +
                '<span class="must2">*  <span id="storeIdText' + index + '"></span></span>' +
        ...
        function PreviewImage1(imgFile) {
            var e = $("#imgPreview1").parent().find("input").attr("name", "bOrContPros[1].photo");
...
// 	      document.getElementById("imgPreview").innerHTML = "<img src='"+path+"'/>"; 
        ...
        function abandonedxm() {
            var v = $("#storeId").val();
            var cusServiceId = $("#cusServiceId").val();
            var content = $("#complaintProblemNrid").val();


            window.location.href = "${ctx}/cusserviceproblem/bizCusServiceProblem/ProjectViewafte?storeId=" + v + "&cusServiceId=" + cusServiceId + "&complaintProblemNr=" + content;
        }


        function sure() {
        	var  beforehandDateIdyemian = $("#beforehandDateIdyemian").val();
        	if(null==beforehandDateIdyemian ||beforehandDateIdyemian==""||undefined==beforehandDateIdyemian){
        		alert("执行时限不能为空");
        		return false;
        	}
        	var  disposefaIdyemina = $("#disposefaIdyemina").val();
//         	alert(disposefaIdyemina)
        	if(null==disposefaIdyemina || disposefaIdyemina=="" || undefined==disposefaIdyemina){
        		alert("方案不能为空");
        		return false;
        		
        	}
//         	alert(beforehandDateIdyemian+":"+disposefaIdyemina)
        	$("#beforehandDatehouId").val(beforehandDateIdyemian);
        	$("#disposefaHouId").val(disposefaIdyemina);

            $("#inputForm").attr("action", "${ctx}/ordercomplan/bizOrderComplaint/saveAllafte");
            $("#inputForm").submit();
        ...
        function submitForm() {
            ...
            if (is == 1) {
                ...
                $("#beforehandDateIdyemian").val($("#executeLimitDihie").val());
...
//                 	alert(result.executeTimeLimit)
	    			...
	    			var executeTimeLimit = '<span  id="spanid1a" class="fl pt8">执行时限：' + result.executeTimeLimit + '</span>'
	               	var executeTimeLimitDivid = '<input  id="executeLimitDihie" type="hidden"  name="executeTimeLimit" value="' + result.executeTimeLimit + '"></input>'
...
<ul class="nav nav-tabs">
    <!-- 		<li><a href="#" onclick="javascript:history.go(-1)">售后问题列表</a></li> -->
    <li class="active"><a href="#">接收售后投诉问题<shiro:hasPermission
...
<form:form id="inputForm" modelAttribute="bizOrderComplaint" action="${ctx}/ordercomplan/bizOrderComplaint/saveAllafte"
           method="post" class="form-horizontal" enctype="multipart/form-data">
    <sys:message content="${message}"/>

    <div class="font0">
        <input id="complaintProblemNrid" name="complaintProblemNr" type="hidden"
               value="${bizOrderComplaint.complaintProblemNr}"/>
        <input id="cusServiceId" name="cusServiceId" type="hidden" value="${bizOrderComplaint.cusServiceId}"/>
        <input id="workOrderCodeId" name="workOrderCode" type="hidden" value="${bizOrderComplaint.workOrderCode}"/>
        <input id="orderId" name="orderId" type="hidden" value="${bizOrderComplaint.order.orderId}"/>
        <input id="disposefaHouId" name="disposefaHou" type="hidden" value="0"/>
        <input id="beforehandDatehouId" name="beforehandDatehou" type="hidden" value="0"/>
            ...
            <div>
                ...
                <ul class="pl152 bg_e tab pt32 pb32 info font0">
                    ...
                    <li class="col3 font0 mb20">
                        ...
                        <form:input id="customerName" path="order.customerName" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                        <label class="f14 col3 ml1em ml110" for="">手机号：</label>
                        <form:input id="customerPhone" path="order.customerPhone" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                    </li>
                    <li class="col3 font0 mb20">
                        <label class="f14 col3" for="">项目经理：</label>
                        <form:input id="itemManager" path="order.itemManager" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                        <label class="f14 col3 ml1em ml110" for="">质检员：</label>
                        <form:input id="orderInspector" path="order.orderInspector" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                    </li>
                    <li class="col3 font0 mb20">
                        ...
                        <form:input id="projectMode" path="order.projectMode" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                        <label class="f14 col3 ml110" for="">工程区域：</label>
                        <form:input id="engineDepartName" path="order.engineDepartName" htmlEscape="false"
                                    maxlength="16" class="itemInput"
                                    readOnly="true"/>
                    </li>
                    <li class="col3 font0 mb20">
                        <label class="f14 col3" for="">订单编号：</label>
                        <form:input id="orderNumber" path="order.orderNumber" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                        <label class="f14 col3 ml110" for="">小区名称：</label>
                        <form:input id="communityName" path="order.communityName" htmlEscape="false" 
                                    class="itemInput"
                                    readOnly="true"/>
                    </li>
                </ul>
                ...
                <div class="pl152 bg_e clearfix">
                    <ul class="col3 f14 mb20 comp">
                        <li class="InfoTit">
                            ...
                            <form:select onchange="delectDiv()" path="complaintSource" disabled="true"
                                         cssStyle="width:190px" id="complaintSource" class="input-medium">
                                <form:option value="4" label="客管部"/>
                                <%-- 							<form:options items="${fns:getDictList('complaintSource_cli')}" itemLabel="label" itemValue="value" htmlEscape="false"/> --%>
                            </form:select>
                    ...
                    <section class="itemWrapper">
                            ...
                            <div class="lab bor_btm_d7 divClass">
                                ...
                                <p class="col3 f14 mb20 clearfix">
                                    ...
                                    <select id="select1" onchange="gradeChange(this)"
                                            name="bOrContPros[1].complaintProblemTypeId" path="complaintProblemNrid"
                                            class="input-medium">
                                    </select>
                                ...
                                <div class="col3 f14 mb20 clearfix">
                                    <span class="must2"> *</span>
                                    <span class="fl pt8">问题事项：</span>
                                    <div id="p1a" onchange="getCheckbox1(this)" class="overflow checkWrap">
                                    </div>
                                    <input id="getInitValue" type="hidden"   value=""></input>
                                </div>
                                 <div class="col3 f14 mb20 clearfix">
	                                <span id="executeTimeLimit1" class="fl pt8"> 执行时限：</span> <span class="fl pt8"> 天</span>
                                ...
                                <div class="col3 f14 pb20 clearfix">
                                    ...
                                    <textarea id="complaintextareaId" class="complainText"
                                              ...
                                              maxLength="300">${bizOrderComplaint.complaintProblemNr}</textarea>
                                </div>
                                <div class="col3 f14 pb20 clearfix">
                                    <span class="fl pt8">上传附件：</span>
                                    <div class="pics font0 clearfix">
                                        <c:forEach items="${picList }" var="picUrl">
                                            <div class="pic camera" href="javascript:void(0)">
                                                <img id="imgPreview${picUrl.xuhao}" src="${picUrl.xuhaourl}" alt="">
                                                <c:if test="${picUrl.cusServicefla == 1}">
                                                <input id="inputPreview${picUrl.xuhao}"  type="hidden" name="bOrContPros[1].photos" value="${picUrl.xuhaourl}">
                                                </c:if>
                                                <input type="file" name="bOrContPros[1].photo" accept="image/*"
                                                       onchange="PreviewImage${picUrl.xuhao}(this)"
                                                       class="inputCoverDiv">
                                                <a class="del_pic" href="javascript:void(0)"></a>
                                            </div>
                                        </c:forEach>
                        ...
                        <div style="background:rgba(0,0,0,.4);z-index:9;" class="g-mask undis" id="subReport2">
                            <div class="alertMask">
								...
								<div class="col3 f14 mb20 clearfix">
	                                <span  class="fl pt8"> 预计处理时间：</span>
	                                <input id="beforehandDateIdyemian" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'0')}else{this.value=this.value.replace(/\D/g,'')}"  name = "beforehandDate" value=""/>
                    				  天 <span class="must2">* <span id='complaintSourceText'></span></span>
                    			 </div>
								 
								 
                                <div class="col3 f14 pb20  clearfix">
                                    <span class="fl pt8">处理方案：</span>
                                    <textarea id="disposefaIdyemina" class="complainText"
                                              name="disposefa" class="complainText"
                                              onkeyup="this.value = this.value.substring(0,100)"
                                              placeholder="最多输入100个汉字"
                                              maxLength="300"  ></textarea> <span class="must2">* <span id='complaintSourceText'></span></span>
                                </div>
                                    <div class="maskBtns clearfix twoBtns">
                                        <a class="maskBtn font33 col_fdfcfa" href="javascript:void(0);"
                                           onclick="sure()">
                                          		  确定
                                        </a>
                                        <a class="maskBtn font33 col_0780ec" href="javascript:void(0);"
                                           onclick="cancel()">取消
                                        </a>
                                    </div>
                                </div>
                    ...
                    <div class="btnWrapper">
                        ...
                        <a class="btn subBtn" href="${ctx}/cusserviceproblem/bizCusServiceProblem/list">返回</a>
                    </div>
                </div>
            </div>
        </section>
