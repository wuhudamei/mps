...
<body>
	<ul class="nav nav-tabs">
			<li><a href="${ctx}/standradmaterialsrecords/bizMaterialsStandardReceiveBillApply/list">标化辅材领取记录列表</a></li>
		...
		<div>
				<div class="col-md-12 column">
					<div >
						<h3>
							<center>
								大美装饰管理平台--标化辅材明细--${ list.materialsStandardReceiveBillCode }
							</center>
						</h3>
					</div>
				</div>
			...
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<c:forEach items="${listStandard}" var="list" varStatus="status">
					<tr>
						...
						<td>
							<c:if test="${ list.maxReceiveNumberSnap==null}">
							0.0
							</c:if>
							<c:if test="${ list.maxReceiveNumberSnap!=null}">
							${list.maxReceiveNumberSnap}
							</c:if>
						</td>
						<td>
							${list.receiveNumber}
