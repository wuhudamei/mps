...
<head>
	...
	<script type="text/javascript">
		...
		function onclickFrom(){
			...
			window.location.href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/from/"+v+"/20/"+date;
		};
		...
		function onclickAbandoned(){
			...
			if(date == ""){
				$("#myNoAbandonedModal").modal('show');
					...
					window.location.href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/from/"+v+"/30/"+date;
					}
			$("#reson").val('');
...
<body>                                    
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/list">灯具和五金申请记录列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizMaterialsStandardReceiveBillApply" action="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/list" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizMaterialsStandardReceiveBill">
			<tr>
				...
				<shiro:hasPermission name="lightmaterialsapplyrecords:bizMaterialsLightReceiveBillApply:edit"><td>
					<c:if test="${bizMaterialsStandardReceiveBill.status =='10' }">
				...
				<form:form id="timeForm" modelAttribute="bizMaterialsStandardReceiveBillApply" action="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/from">
				...
				<a href="javascript:void(0)" class="btn" onclick="onclickClean()">关闭</a>
						...
						<a href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/detail/${bizMaterialsStandardReceiveBill.id}/2">详情</a>
					</c:if>
					<c:if test="${bizMaterialsStandardReceiveBill.status =='20' }">
						<a href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/detail/${bizMaterialsStandardReceiveBill.id}/1">详情</a>
					</c:if>
					<c:if test="${bizMaterialsStandardReceiveBill.status =='30' }">
						<a href="${ctx}/standradmaterialsrecords/bizMaterialsLightReceiveBillApply/detail/${bizMaterialsStandardReceiveBill.id}/1">详情</a>
					</c:if>
