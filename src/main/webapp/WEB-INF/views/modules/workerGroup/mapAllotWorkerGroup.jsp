	...
	<body>
		...
		//页面加载 发送ajax 查出工人组marker
			window.onload = function() {
				loading("查询中,请稍等...");
				$.ajax({
					url : "${ctx}/AllotWorkerGroup/allotWorkerGroup/mapAllot?packageId=${taskpakcage.id}&packageCode=${taskpakcage.packageCode}&itemManageName=${workgroupVo.itemManageName}&itemManagerId=${workgroupVo.itemManagerId}",
					type:"post",
		...
		//包括 : 元素内容,标签,节点,事件,样式 等
		    ...
		    function closeInfoWindow() {
				...
				function getWorkerGroup(){
					...
					$.ajax({
						url : "${ctx}/AllotWorkerGroup/allotWorkerGroup/mapAllot?packageId=${taskpakcage.id}"+"&startDate="+start+"&endDate="+end+"&scopeDistance="+scopeDistance+"&starLevel="+starLevel+"&groupName="+groupName+"&itemManageName="+itemManageName+"&packageCode=${taskpakcage.packageCode}",
						success : function(data) {
