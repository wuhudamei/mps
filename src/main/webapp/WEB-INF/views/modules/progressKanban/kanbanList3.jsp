...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/progresskanban3/progressKanban3/preList">工程进度看板</a></li>
	...
	<form:form id="searchForm" modelAttribute="progressKanban3" action="${ctx}/progresskanban3/progressKanban3/list" method="post" class="breadcrumb form-search" onsubmit="return checkStoreID();">
		...
		<ul class="ul-form">
			...
			<li><label>实际开工日期：</label>
				<input name="actualBegin" type="text" id="actualBegin" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${progressKanban3.actualBegin}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				...
				<input name="actualEnd" type="text" id="actualEnd" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${progressKanban3.actualEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="progressKanban" varStatus="status">
			<tr>
				...
				<c:if test="${npList.nodeIndex == csList.sort }">
					<table>
					<tbody>
						<td >
							<tr><td><fmt:formatDate type="date" value="${npList.planDoneDate }"/></td>
							<td><fmt:formatDate type="date" value="${npList.realDoneDate }"/></td>
							<td>${npList.realLessPlan }</td></tr>
						</td>
					</tbody>
					</table>
				</c:if>
