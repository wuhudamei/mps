...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="${ctx}/scheduling/orderTaskpackage/workerListNew">派工监控</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="orderTaskpackage"
		action="${ctx}/scheduling/orderTaskpackage/workerList2New" method="post"
		...
		<tbody>
			<c:forEach items="${page.list}" var="orderTaskpackage">
				<tr>
					...
					<td><fmt:formatDate value="${orderTaskpackage.dispatchTime}"
					...
					<td>
						<c:if test="${orderTaskpackage.isScrap eq 1}">是</c:if>
						<c:if test="${orderTaskpackage.isScrap ne 1}">否</c:if>
					</td>
					<td><a
					...
					<shiro:hasPermission name="scheduling:orderTaskpackage:edit">
						<td style="width: 70px;">
							<c:if test="${orderTaskpackage.isOvertime=='1'}">
								<a href="${ctx}/AllotWorkerGroup/allotWorkerGroup/listNew?packageId=${orderTaskpackage.id}&isRedistribute=1&packageCode=${orderTaskpackage.packageCode}&itemManageName=${orderTaskpackage.itemCustomer}&projectMode=${orderTaskpackage.projectMode}">重新分派</a>
							</c:if>
