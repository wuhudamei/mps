...
function checkStartScore(){
	...
	if(star == null || star=='' ||storeId == null || storeId=='' || projectMode ==null || projectMode=='' ||startScore ==null || startScore == '' || endScore == null || endScore == ''){
	  ...
	  $.ajax({
             type: "get",
             url: "${ctx}/starSetting/starRatingSetting/findIsNotCopyStar?storeId="+storeId+"&projectMode="+projectMode+"&star="+star+"&startScore="+startScore+"&endScore="+endScore,
             async: true,
             success: function(data){
                        if(data.startScore!=""&&data.startScore!=null){
                        	alertx("已经存在起始分数或者结束分数的配置项了（"+data.startScore+"~"+data.endScore+"），开始分数和结束分数是不允许重复的，请您修改开始分数和结束分数。")
                        	return;
                        }
                        else{
                        	$("#inputForm").submit();
...
<body>
	...
	<form:form id="inputForm" modelAttribute="bizStarSetting" action="${ctx}/starSetting/starRatingSetting/update" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="star" class="input-xlarge required"  htmlEscape="false" disabled="true">
