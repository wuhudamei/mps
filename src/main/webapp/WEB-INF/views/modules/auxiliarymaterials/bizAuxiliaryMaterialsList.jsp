	...
	<script type="text/javascript">
		...
		function exportExcel(date){
			if(date==''){
				...
				$("#searchForm").submit();
	            $("#searchForm").attr("action", "${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/list");
			}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/">辅料管理列表</a></li>
		<shiro:hasPermission name="auxiliarymaterials:bizAuxiliaryMaterials:edit"><li><a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/form">辅料管理添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="bizAuxiliaryMaterials" action="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>状态</th>
				<th>品牌</th>
				<shiro:hasPermission name="auxiliarymaterials:bizAuxiliaryMaterials:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
...
<%-- 				<shiro:hasPermission name="auxiliarymaterials:bizAuxiliaryMaterials:edit"><td>
    				<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/form?id=${bizAuxiliaryMaterials.id}">修改</a>
					<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/delete?id=${bizAuxiliaryMaterials.id}" onclick="return confirmx('确认要删除该辅料管理吗？', this.href)">删除</a>
				</td></shiro:hasPermission> --%>
				<shiro:hasPermission name="auxiliarymaterials:bizAuxiliaryMaterials:edit"><td>
    				<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/form?id=${bizAuxiliaryMaterials.id}">修改基本信息</a>
    				<a href="${ctx}/auxiliarymaterialssupplier/bizAuxiliaryMaterialsSupplierRel/list?auxiliaryMaterialsId=${bizAuxiliaryMaterials.id}">修改工人结算价</a>
					<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/enable?id=${bizAuxiliaryMaterials.id}" onclick="return confirmx('确认要停用吗？', this.href)">

                        <c:if test="${bizAuxiliaryMaterials.status==1}">
							<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/enable?id=${bizAuxiliaryMaterials.id}" onclick="return confirmx('确认要停用吗？', this.href)">停用</a>
						</c:if>
						<c:if test="${bizAuxiliaryMaterials.status==0}">
							<a href="${ctx}/auxiliarymaterials/bizAuxiliaryMaterials/enable?id=${bizAuxiliaryMaterials.id}" onclick="return confirmx('确认要启用吗？', this.href)">启用</a>
						</c:if>
