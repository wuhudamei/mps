...
public class ApplyLightMaterialController {
	...
	@RequestMapping(value="lightMaterialList")
	public String standarMaterialList(String villageAndName,HttpServletRequest request, Model model){
		...
		List<ApplyStandardMaterial> list = applyStandardMaterialService.findOrderListById(am,"2");
	...
	public String applyStandarMaterialList(@PathVariable String orderId,HttpServletRequest request, Model model){
		...
		model.addAttribute("orderId", orderId);
		return "mobile/modules/Manager/materialManagement/lightApply";
		}
	}
	@Transactional
	...
	public String saveStandardBill(Integer shippingType,Double shippingFee,String random,String orderId,String[] id,String[] number,HttpServletRequest request, Model model ){
		...
		return "mobile/modules/Manager/materialManagement/lightDetails";
		}
	}
	...
	public String record(String orderId,HttpServletRequest request, Model model){
		...
		return"mobile/modules/Manager/materialManagement/lightRecordList";
	}
	...
	private double getSquare(String orderArea,List<String> ruleCodeNumber){
		  Double area = Double.valueOf(orderArea);
		if (ruleCodeNumber==null) {
		}else{
			if(area>71){
				if(ruleCodeNumber.contains("2")){
					return Math.ceil((area*8%100==0)?area*8/100:area*8/100+1);
				}else if(ruleCodeNumber.contains("4")){
					return  Math.ceil((area*16%100==0)?area*16/100:area*16/100+1);
				}
			}else{
				if(ruleCodeNumber.contains("1")){
					return 71*8/100+1;
				}else if(ruleCodeNumber.contains("3")){
					return 71*16/100+1;
				}
			}
		}
