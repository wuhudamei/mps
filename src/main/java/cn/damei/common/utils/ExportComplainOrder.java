...
public class ExportComplainOrder {
	
	public static HSSFWorkbook exportComplainOrder(List<ScoreOrderComplain> list){
		HSSFWorkbook wb = new HSSFWorkbook();// 创建一个Excel文件
		HSSFSheet sheet = wb.createSheet("客诉统计表");// 创建一个Excel的Sheet
		// 定义样式
		...
		try {  
            ...
            cell.setCellValue(new HSSFRichTextString("客诉内容"));
            cell = sheet.getRow(1).getCell(9);  
            cell.setCellValue(new HSSFRichTextString("客诉员"));
            cell = sheet.getRow(1).getCell(10);  
            cell.setCellValue(new HSSFRichTextString("客诉时间"));
// ---------------------------3.合并单元格------------------------------  
            ...
            for (int i= 0; i<list.size();i++) {
            	row = sheet.createRow(i+3);
					...
					row.createCell(2).setCellValue(list.get(i).getComplainCode());
					row.createCell(3).setCellValue(list.get(i).getOrderNumber());
					row.createCell(4).setCellValue(list.get(i).getLabel());
					...
					row.createCell(8).setCellValue(list.get(i).getComplainContent());
					row.createCell(9).setCellValue(list.get(i).getEmployeeName());
					row.createCell(10).setCellValue(sf.format(list.get(i).getComplainTime()));
				}
        } catch (Exception e) {  
            e.printStackTrace();  
        }
