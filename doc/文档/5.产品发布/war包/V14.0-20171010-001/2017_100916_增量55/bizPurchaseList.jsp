...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/purchase/bizPurchase/list">辅材申请列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bizPurchaseVo" action="${ctx}/purchase/bizPurchase/list" method="post" class="breadcrumb form-search">
			...
			<tr>
				...
				<shiro:hasPermission name="purchase:bizPurchase:edit"><td>
					...
					<c:if test="${bizPurchaseVo.status eq 10 }">
						<a href="${ctx}/purchase1/purchase1/reviseAuxiliaryStatus?id=${bizPurchaseVo.id}&status=40">
							转给供应商
						</a>
					</c:if>
					<a href="${ctx}/purchase/bizPurchase/exportExcel?id=${bizPurchaseVo.id}">
							导出
						</a>
					<c:if test="${bizPurchaseVo.status eq 10 || bizPurchaseVo.status eq 40}">
						<a href="${ctx}/purchase1/purchase1/reviseAuxiliaryStatus?id=${bizPurchaseVo.id}&status=21">
							废弃
						</a>
					</c:if>
