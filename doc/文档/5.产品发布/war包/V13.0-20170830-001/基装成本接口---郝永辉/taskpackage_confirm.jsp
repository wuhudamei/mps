...
<head>
	...
	<link rel="stylesheet" type="text/css" href="${ctxStatic}/mobile/modules/Manager/css/check_confirm.css"/>
...
<body>
	<div class="g-check" id="aboveId">
		...
		<form id="submitForm">
		<input type="hidden" value="${orderTaskpackageVo.id}" name="orderTaskpackageId"/>
		<input type="hidden" value="${orderTaskpackageVo.orderId}" name="orderId"/>
		<section class="check_section padtop8">
			...
			<ul class="lists shadow">
				...
				<li class="line_hgt6 clearfix">
					...
					<p class="p_right">${orderTaskpackageVo.packageName}</p>
				</li>
				<li class="line_hgt6 clearfix">
				...
				<li class="line_hgt6 clearfix">
					<span class="span_left">工人组长：</span>
				...
				<li class="line_hgt6 clearfix">
					...
					<p class="p_right"><c:if test="${not empty orderTaskpackageVo.coveredArea}">${orderTaskpackageVo.coveredArea}平米</c:if></p>
				</li>
			</ul>
			...
			<div class="box-bottom">
				...
				<ul class="lists shadow">
					<li class="line_hgt6 clearfix">
						<span class="span_left">验收日期：</span>
						<p class="p_right">
							<input id="txtBeginDate" class="date" type="text" name="checkedDate" placeholder="请输入日期" data-lcalendar="2000-01-01,2000-01-01" />
						</p>
					</li>
					<li class="line_hgt6 clearfix">
						<span class="span_left">质量检验：</span>
						<p class="p_right">
							<span class="radio_span marrgt1">
								<input type="radio" id="yes" name="isQualified" value="1">
								<label data-name="quality" for="yes" class="checked">合格</label>
							</span>
							<span class="radio_span">
								<input type="radio" id="no" name="isQualified" value="0">
								<label data-name="quality" for="no" id="qualifiedNo">不合格</label>
							</span>
						</p>
					</li>
					<li class="line_hgt6 clearfix">
						<span class="span_left">实际工期：</span>
						<p class="p_right">
							<span class="radio_span marrgt04">
								<input type="radio" id="on" name="isDelay" value="0">
								<label data-name="ontime" for="on" class="checked" id="delayYes">按时完成</label>
							</span>
							<span class="radio_span">
								<input type="hidden" id="delayAmerce" name="delayAmerce"/>
								<input type="radio" id="out" name="isDelay" value="1">
								<label data-name="ontime" for="out" id="delayNo">延期<input class="days wid100" type="text" name="delayDays" id="delayDays" oninput="this.value=this.value.replace(/[^\d\.]/g,'');initMoney();">天</label>
							</span>
						</p>
					</li>
					<li class="line_hgt6 clearfix">
						<span class="span_left">管理处罚：</span>
						<p class="p_right">
							<span class="radio_span blk">
								<input type="radio" id="one" name="isManagePunish" value="0">
								<label data-name="punish" for="one" class="checked" id="punishYes">无</label>
							</span>
							<span class="radio_span">
								<input type="radio" id="two" name="isManagePunish" value="1">
								<label data-name="punish" for="two" id="punishNo">有
								<input class="days wid100 margin0" type="text" maxlength="10" id="punishAmerce" name="punishAmerce" oninput="this.value=this.value.replace(/[^\d\.]/g,'');initMoney()">元</label>
								<input class="days wid284 font24 col_b4" type="text" id="punishReason" name="punishReason" placeholder="若有处罚，请说明处罚原因" />
							</span>
						</p>
					</li>
					<li class="line_hgt6 clearfix">
						<span class="span_left">质检罚款：</span>
						<p class="p_right">
							${qcAmount}元<input type="hidden" id="qcAmount" name="qcPunishMoneyAmount" value="${qcAmount}"/>
						</p>
					</li>
					<li class="line_hgt6 clearfix">
						<span class="span_left">公司扣款：</span>
						<p class="p_right">
						  <c:if test="${settle.companyDeductAmount == null}">
						  -0.0元
						  </c:if>
						  <c:if test="${settle.companyDeductAmount != null}">
						  -${settle.companyDeductAmount}元
						  </c:if>
						  <input type="hidden" id="companyDeductAmount" name="companyDeductAmount" value="${settle.companyDeductAmount}"/>
						</p>
					</li>
					<c:if test="${settle.companyDeductAmount != null}">
					<li class="line_hgt6 clearfix">
			...
			<c:if test="${fn:length(bizEvalIndexList) > 0}">
				...
				<div class="box-bottom">
					<ul class="lists shadow font0">
						<c:forEach items="${bizEvalIndexList}" var="bizEvalIndex" varStatus="status">
							<li class="mar_btm24 clearfix a">
								<input type="hidden" name="bizEvalActivityIndexList[${status.index}].id" value="${bizEvalIndex.id}"/>
								<input type="hidden" name="bizEvalActivityIndexList[${status.index}].evalTotalScore" value="${bizEvalIndex.evalTotalScore}"/>
								<input type="hidden" id="selectCount${status.index}" name="bizEvalActivityIndexList[${status.index}].selectCount"/>
								<span class="col_3 font28 mar_ght32 left-small" id="indexName${status.index}">${bizEvalIndex.indexName}</span>
						...
						<div class="advise-area font0 col_9">
							<textarea class="advise" name="evalFeedback" placeholder="请输入您的反馈意见"></textarea>
						</div>
					</ul>
				</div>
			</c:if>
			...
			<ul class="lists shadow project_bars">
			    <div class="lists font0 shadow">
				 <span class="font28 col6">
					<span class="col6">工料费结算总金额：</span>
					<input id="laborAuxiliaryMaterialsSettleAmount" name="laborAuxiliaryMaterialsSettleAmount" value="" class="days wid200 text_grey" readonly type="text">元
				 </span>
				 <br />
				 <span class="font28 col6">
					<span class="col6">人工费结算总金额：</span>
					<input id="laborSettleAmount" name="laborSettleAmount" value="" class="days wid200 text_grey" readonly type="text">元
				 </span>
				 <br />
				 <span class="font28 col6">
					<span class="col6">辅料费结算总金额：</span>
					<input id="auxiliaryMaterialsSettleAmount" name="auxiliaryMaterialsSettleAmount" value="" class="days wid200 text_grey" readonly type="text">元
				 </span>
			    </div>
			   
				<c:forEach items="${taskProcedureList}" var="taskProcedure" varStatus="status">
					<li class="project_list salary btm_border mar_top20 undis">
						<ul>
							<p class="line_hgt6 font28 col3 clearfix">${status.index+1}、${taskProcedure.procedureName}- 单位：${taskProcedure.measurementUnitLabel}</p>
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									<span class="col6 font28">预算工程量：</span>
									<input type="hidden" value="${taskProcedure.id}" name="orderTaskProcedure[${status.index}].id"/>
									<span class="col3 font28">${taskProcedure.budgetNumber}</span>
									<input type="hidden" id="budgetNumber${status.index}" value="${taskProcedure.budgetNumber}"/>
								</span>
								<span class="pro_span font0">
									<span class="col6 font28">实际工程量：<input class="days wid100" type="text" id="realNumber${status.index}" name="orderTaskProcedure[${status.index}].realNumber" value="${taskProcedure.budgetNumber}" oninput="this.value=this.value.replace(/[^\d\.]/g,'');getTotal(${status.index},this)"></span>
								</span>
							</li>
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									<span class="col6 font28">工料费单价：</span>
									<span class="col3 font28">${taskProcedure.synthesizePrice}元</span>
									<input type="hidden" id="synthesizePrice${status.index}" value="${taskProcedure.synthesizePrice}"/>
									<input type="hidden" id="procedureName${status.index}" value="${taskProcedure.procedureName}"/>
								</span>
								<span class="pro_span font0">
									<span class="col6 font28">工料费金额：</span>
									<span class="col3 font28" id="laborAuxiliaryMaterialsSettleAmount${status.index}">${taskProcedure.laborAuxiliaryMaterialsBudgetAmount}元</span>
									<input type="hidden" id="laborAuxiliaryMaterialsSettleAmountPrice${status.index}" name="orderTaskProcedure[${status.index}].laborAuxiliaryMaterialsSettleAmount" value="${taskProcedure.laborAuxiliaryMaterialsBudgetAmount}"/>
								</span>
							</li>
							...
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									...
									<span class="col3 font28">${taskProcedure.laborPrice}元</span>
									<input type="hidden" id="laborPrice${status.index}" value="${taskProcedure.laborPrice}"/>
									<input type="hidden" id="procedureName${status.index}" value="${taskProcedure.procedureName}"/>
								</span>
								<span class="pro_span font0">
									<span class="col6 font28">人工费金额：</span>
									<span class="col3 font28" id="laborSettleAmount${status.index}">${taskProcedure.laborDudgetAmount}元</span>
									<input type="hidden" id="laborSettleAmountPrice${status.index}" name="orderTaskProcedure[${status.index}].laborSettleAmount" value="${taskProcedure.laborDudgetAmount}"/>
								</span>
							</li>
							...
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									...
									<span class="col3 font28">${taskProcedure.accessoriesPrice}元</span>
									<input type="hidden" id="accessoriesPrice${status.index}" value="${taskProcedure.accessoriesPrice}"/>
									<input type="hidden" id="procedureName${status.index}" value="${taskProcedure.procedureName}"/>
								</span>
								<span class="pro_span font0">
									<span class="col6 font28">辅料费金额：</span>
									<span class="col3 font28" id="auxiliaryMaterialsSettleAmount${status.index}">${taskProcedure.auxiliaryMaterialsBudgetAmount}元</span>
									<input type="hidden" id="auxiliaryMaterialsSettleAmountPrice${status.index}" name="orderTaskProcedure[${status.index}].auxiliaryMaterialsSettleAmount" value="${taskProcedure.auxiliaryMaterialsBudgetAmount}"/>
								</span>
							</li>
							
							<li class="line_hgt6 mar_btm20 clearfix">
								<span class="span_left col6">备 注 信 息  ：</span>
								<p class=""><input class="days wid370 font28" type="text" id="remark${status.index}" name="orderTaskProcedure[${status.index}].remarks" value="${taskProcedure.remarks}"/></p>
							</li>
						</ul>
					</li>
				</c:forEach>
		...
		<div class="box-bottom mar_top100 pad_btm2">
			...
			<div class="lists font0 shadow">
				<span class="font28 col6">
					...
					<input class="days wid200 text_grey" type="text" readonly="true" id="auxiliaryAmount" name="auxiliaryMaterialsAmount">元
				</span>
				<a class="auxi" href="javascript:void(0)" onclick="showAuxiliary()">获取辅料</a>
			</div>
			...
			<div class="lists font0 shadow">
				<span class="font28 col6">
					...
					<input class="days wid200 text_grey" type="text" readonly="true" id="sandAmount" name="sandCementAmount"/>元
				</span>
				<a class="auxi" href="javascript:void(0)" onclick="showSand()">获取沙子水泥</a>
			</div>
			</c:if>
			<h3 class="dot">上缴质保金</h3>
			<div class="lists font0 shadow">
				<span class="font0 col6 marrgt09">
					<span class="font28 col6">工人姓名：</span>
					<span class="font28 col3">${orderTaskpackageVo.realName}</span>
					<input type="hidden" value="${orderTaskpackageVo.groupId}" name="groupId"/>
					<input type="hidden" value="${orderTaskpackageVo.empGroupid}" name="empGroupid"/>
					<input type="hidden" value="${orderTaskpackageVo.taskPackageTemplatId}" name="taskPackageTemplatId"/>
					<input type="hidden" value="${orderTaskpackageVo.guaranteeMoneyAmountTotal}" name="guaranteeMoneyAmountTotal"/>
					<input type="hidden" value="${orderTaskpackageVo.gualityGuaranteeType}" name="gualityGuaranteeType"/>
				</span>
				<span class="font0 col6">
					<span class="font28 col6">上缴金额：</span>
					<span class="font28 col6"><input class="days wid126 text_grey"  readonly="true" type="text" id="guaranteeMoneyAmount" name="guaranteeMoneyAmount">元</span>
				</span>
			...
			<div class="lists font0 shadow">
				<span class="font28 col6">
					<span class="col6">奖励金额：</span>
					<input class="days wid200 text_grey" type="text" readonly="true" placeholder="待核算">元
				</span>
			</div>
			...
			<div class="lists font0 shadow">
				...
				<!-- <p class="col9 font24 mar_top20 line_hgt4">备注：结算金额=（结算总金额-辅材-沙子水泥-工期扣款-管理扣款-质检罚款-质保金-公司扣款）</p> -->
			</div>
			<h3 class="dot pos_ret">
				分配薪酬
			</h3>
				...
				<c:forEach items="${empList}" var = "emp" varStatus="status">
					<li class="salary mar_top20 btm_border">
						<ul>
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									<span class="col6 font28">工人姓名：</span>
									<span class="col3 font28" id="realName${status.index}">${emp.realName}</span>
									<input type="hidden" value="${emp.id}" name="settlementDetail[${status.index}].employeeId"/>
									<input type="hidden" value="${emp.isLeader}" name="settlementDetail[${status.index}].isLeader"/>
								</span>
								<span class="font28 col6">
									<span class="col6">分配金额：</span>
									<input class="days wid126" type="text" id="paymentAmount${status.index}" name="settlementDetail[${status.index}].paymentAmount" oninput="this.value=this.value.replace(/[^\d\.]/g,'')">元
								</span>
							</li>
							<li class="line_hgt6 mar_btm20 clearfix">
								<span class="span_left col6">备 注 信 息  ：</span>
								<p class=""><input class="font28 days wid370" type="text" name="settlementDetail[${status.index}].remarks"/></p>
							</li>
						</ul>
					</li>
				</c:forEach>
		...
		<!-- 辅料用料弹框 -->
		<section id="auxiliaryModel" class="g-auxiliary_choose undis">
			<!-- <h3 class="mask_tit">辅料用量</h3> -->
			<div class="show_sec">
				<ul>
					<c:forEach items="${auxiliaryList}" var = "auxiliary" varStatus="status">
						<li class="shadow">
							<div class="img_container">
								<img src="${root}${auxiliary.picUrl}" alt="goods" onerror="nofind()" />
							</div>
							<p class="brand">${auxiliary.auxiliaryMaterialsName}</p>
							<input type="hidden" id="auxiliaryMaterialsName${status.index}" value="${auxiliary.auxiliaryMaterialsName}">
							<p class="format">
								规格：${auxiliary.specifications}
								<span class="price">送货数量：${auxiliary.totalAuxiMateCount}${auxiliary.measurementUnitLabel}</span>
								<input type="hidden" id="totalAuxiMateCount${status.index}" value="${auxiliary.totalAuxiMateCount}">
								<input type="hidden" value="${auxiliary.auxiliaryMaterialsNo}" name="auxiliaryMaterials[${status.index}].auxiliaryMaterialsNo"/>
							</p>
							<p class="labeler">使用数量：</p>
							<input type="hidden" id="userCountTotal${status.index}" value="${auxiliary.totalAuxiMateCount}"/>
							<div class="clearfix shopcount">
								<a class="reduce" href="javascript:;"></a>
								<input type="text" id="useCount${status.index}" name="auxiliaryMaterials[${status.index}].usedCount" readonly value="${auxiliary.totalAuxiMateCount}" maxlength="11"/>
								<a class="plus" href="javascript:;"></a>
							</div>
							<span class="price_sin col_red">¥${auxiliary.laborPrice}</span>
							<input type="hidden" id="auxiliarylaborPrice${status.index}" value="${auxiliary.laborPrice}" name="auxiliaryMaterials[${status.index}].price" />
							<input type="hidden" id="supplierPrice${status.index}" value="${auxiliary.supplierPrice}" name="auxiliaryMaterials[${status.index}].supplierPrice" />
							<input type="hidden" id="wangzhenPrice${status.index}" value="${auxiliary.wangzhenPrice}" name="auxiliaryMaterials[${status.index}].wangzhenPrice" />
							<input type="hidden"  value="0" name="auxiliaryMaterials[${status.index}].isSandCement" />
						</li>
					</c:forEach>
				</ul>
			</div>
			<footer>
				<div>
					<p class="col_red">合计：¥<span id="auxiliaryTotal">0</span></p>
					<input type="hidden" id="auxiliarySupplierTotal" />
					<input type="hidden" id="auxiliaryWangzhenTotal" />
					<p class="goods">共使用<span id="auxiliaryCount">0</span>种商品</p>
				</div>
				<a class="choose_btn" href="javascript:void(0)" onclick="chooseAuxiliary()">确认用量</a>
			</footer>
		</section>
		
		<!-- 沙子水泥用料弹框 -->
		<section id="sandModel" class="g-auxiliary_choose undis">
			<div class="show_sec">
				<ul>
					<c:forEach items="${sandList}" var = "auxiliary" varStatus="status">
						<li class="shadow">
							...
							<p class="brand">${auxiliary.auxiliaryMaterialsName}</p>
							<input type="hidden" id="sandMaterialsName${status.index}" value="${auxiliary.auxiliaryMaterialsName}">
							<p class="format">
								规格：${auxiliary.specifications}
								<span class="price">送货数量：${auxiliary.totalAuxiMateCount}${auxiliary.measurementUnitLabel}</span>
								<input type="hidden" id="sandtotalAuxiMateCount${status.index}" value="${auxiliary.totalAuxiMateCount}">
								<input type="hidden" value="${auxiliary.auxiliaryMaterialsNo}" name="sandMaterials[${status.index}].auxiliaryMaterialsNo"/>
							</p>
							<p class="labeler">使用数量：</p>
							<input type="hidden" id="sanduserCountTotal${status.index}" value="${auxiliary.totalAuxiMateCount}"/>
							<div class="clearfix shopcount">
								<a class="reduce" href="javascript:;"></a>
								<input type="text" id="sanduseCount${status.index}" name="sandMaterials[${status.index}].usedCount" readonly value="${auxiliary.totalAuxiMateCount}" maxlength="11"/>
								<a class="plus" href="javascript:;"></a>
							</div>
							<span class="price_sin col_red">¥${auxiliary.laborPrice}</span>
							<input type="hidden" id="sandlaborPrice${status.index}" value="${auxiliary.laborPrice}" name="sandMaterials[${status.index}].price" />
							<input type="hidden" id="sandsupplierPrice${status.index}" value="${auxiliary.supplierPrice}" name="sandMaterials[${status.index}].supplierPrice" />
							<input type="hidden" id="sandwangzhenPrice${status.index}" value="${auxiliary.wangzhenPrice}" name="sandMaterials[${status.index}].wangzhenPrice" />
							<input type="hidden"  value="1" name="sandMaterials[${status.index}].isSandCement" />
						</li>
					</c:forEach>
				</ul>
			</div>
			<footer>
				<div>
					<p class="col_red">合计：¥<span id="sandTotal">0</span></p>
					<input type="hidden" id="sandSupplierTotal" />
					<input type="hidden" id="sandWangzhenTotal" />
					<p class="goods">共使用<span id="sandCount">0</span>种商品</p>
				</div>
				<a class="choose_btn" href="javascript:void(0)" onclick="chooseSand()">确认用量</a>
			</footer>
		</section>
	...
	function run_waitMe(effect,text){
		...
		$(function(){
			...
			// 获取辅料弹框
			/* $('.auxi').click(function(){
				$('.g-auxiliary_choose').removeClass('undis');
			...
			$('.footer_mask .footer1 a').on('click', function(){
				...
				if (bool) {
					...
					if($(this).hasClass('check_btn_a')){
						...
						if(auxiliaryAmount == undefined || auxiliaryAmount == null || auxiliaryAmount == ""){
						...
						if(boo){
							...
							var workerGroupSettleAmount = $("#workerGroupSettleAmount").val();
							if(parseFloat(workerGroupSettleAmount) != parseFloat(paymentAmountSum).toFixed(2)){
						...
						$.ajax({  
				            type: "post",  
				            url: "${ctx}/app/manager/orderTaskpackageSettlement",       
				            ...
				            error: function(data) {  
				            	globalUtil.fn.alert('结算单生成成败!',2.0);
				            }  
				        });
					}
				}
			...
			$("#gongchengCheck").click(function(){
				...
				$(this).unbind();
				$.post("${ctx}/app/manager/submitGongcheng",$("#submitForm").serialize(),function(data){
					if(data == "success"){
						globalUtil.fn.alert("提交工程量成功!",2.0);
						window.location.href="${ctx}/app/manager/packageSettlementList";
					}else if(data == "repeat"){
						globalUtil.fn.alert("您已提交工程量到质检员复核，请不要重复操作!",2.0);
					}else{
		...
		// 辅料计算价格
    	function getAuxiliaryTotalPrice(){
	  		...
	  		$("#auxiliaryTotal").html(parseFloat(totalPrice).toFixed(2));
		...
		function initMoney(){
			...
			if('${orderTaskpackageVo.gualityGuaranteeType}' == 3){
				...
				var bizGuaranteeMoneyBalance = '${orderTaskpackageVo.guaranteeMoneyBalance}';
				...
				if(parseFloat(bizGuaranteeMoneyBalance) >= 2000){ // 累计金额大于2000，质保金为0
					guaranteeMoneyAmount = 0;
				}else{
					if((parseFloat(bizGuaranteeMoneyBalance) + amount) > 2000){
						guaranteeMoneyAmount = (amount - ((parseFloat(bizGuaranteeMoneyBalance) + amount) - parseFloat(2000))).toFixed(2);
					}else{
		...
		function chooseAuxiliary(){
			var length = '${fn:length(auxiliaryList)}';
	  		...
	  		if(!bool){
	  			$('.g-auxiliary_choose').addClass('undis');
				$('.check_section').removeClass('undis');
				$("#auxiliaryAmount").val($("#auxiliaryTotal").html());
