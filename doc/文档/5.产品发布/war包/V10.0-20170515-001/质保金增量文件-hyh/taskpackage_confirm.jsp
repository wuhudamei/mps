...
<body>
					...
					<li class="line_hgt6 clearfix">
						...
						<p class="p_right">
							...
							<span class="radio_span">
								...
								<label data-name="punish" for="two" id="punishNo">有<input class="days wid100 margin0" type="text" maxlength="10" id="punishAmerce" name="punishAmerce" oninput="this.value=this.value.replace(/[^\d\.]/g,'');initMoney()">元</label>
					...
					<ul class="lists shadow font0">
						<c:forEach items="${bizEvalIndexList}" var="bizEvalIndex" varStatus="status">
							<li class="mar_btm24 clearfix a">
								...
								<span class="col_3 font28 mar_ght52" id="indexName${status.index}">${bizEvalIndex.indexName}</span>
				...
				<c:forEach items="${taskProcedureList}" var="taskProcedure" varStatus="status">
					<li class="project_list salary btm_border mar_top20 undis">
						<ul>
							...
							<li class="line_hgt6 clearfix">
								<span class="pro_span font0">
									<span class="col6 font28">综合　单价：</span>
								...
								<span class="pro_span font0">
									<span class="col6 font28">结　算　价：</span>
									<span class="col3 font28" id="total${status.index}">${taskProcedure.synthesizePrice*taskProcedure.budgetNumber}元</span>
									<input type="hidden" id="totalPrice${status.index}" name="orderTaskProcedure[${status.index}].total" value="${taskProcedure.synthesizePrice*taskProcedure.budgetNumber}"/>
								</span>
							</li>
				...
				<ul>
					<c:forEach items="${auxiliaryList}" var = "auxiliary" varStatus="status">
						<li class="shadow">
							...
							<input type="hidden" id="laborPrice${status.index}" value="${auxiliary.laborPrice}" name="auxiliaryMaterials[${status.index}].price" />
	...
	function run_waitMe(effect,text){
		...
		function getTotal(index,obj){
			...
			$("#total"+index).html((parseFloat(synthesizePrice) * parseFloat(currentRealNumber)).toFixed(2) + "元");
			$("#totalPrice"+index).val((parseFloat(synthesizePrice) * parseFloat(currentRealNumber)).toFixed(2));
			
			initMoney();
		}
		
		// 初始化时显示质保金(结算总金额 – 辅材金额 –沙子水泥 – 工期扣款 – 管理扣款)
		function initMoney(){
			var length = ${fn:length(taskProcedureList)};
			// 结算总金额
			var allAmount = 0;
